...
// list every module that has a hostbuzz/gen trampoline; magusModules in the bindings
...
// TestBuzzBindingsMatchHostModules guards against the hostbuzz/gen trampolines
...
func TestBuzzBindingsMatchHostModules(t *testing.T) {
	...
	for _, m := range std.All() {
		...
		if !ok {
			// Hand-built modules (e.g. "magus") have no hostbuzz/gen trampoline.
			continue
		}
		mod := reg(ctx, sess)
		...
		for _, meth := range m.Methods {
			...
			_, ok := mod.MapGet(key)
			assert.Truef(t, ok, "buzz %s.%s is missing (host declares it as %q); hostbuzz/gen has drifted from std.Module",
