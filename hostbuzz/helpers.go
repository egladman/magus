...
func Str(args []buzz.Value, n int) string {
	if n >= len(args) {
		return ""
	}
	if v := args[n]; v.IsStr() {
...
func Int(args []buzz.Value, n, def int) int {
	if n >= len(args) {
		return def
	}
	switch v := args[n]; {
...
func Float(args []buzz.Value, n int, def float64) float64 {
	if n >= len(args) {
		return def
	}
	switch v := args[n]; {
...
func Bool(args []buzz.Value, n int, def bool) bool {
	if n >= len(args) {
		return def
	}
...
func StrSlice(args []buzz.Value, n int) []string {
	if n >= len(args) {
		return nil
	}
...
func strSliceFromValue(v buzz.Value) []string {
	if !v.IsList() {
		return nil
	}
	items := v.ListItems()
...
func StrMap(args []buzz.Value, n int) map[string]string {
	if n >= len(args) {
		return nil
	}
	v := args[n]
...
func AnyMap(args []buzz.Value, n int) map[string]any {
	if n >= len(args) {
		return nil
	}
	v := args[n]
...
func Any(args []buzz.Value, n int) any {
	if n >= len(args) {
		return nil
	}
...
func VariadicStr(args []buzz.Value, n int) []string {
	if n >= len(args) {
		return nil
	}
...
func CallbackArg(sess *buzz.Session, args []buzz.Value, n int) std.Callback {
	if n >= len(args) {
		return nil
	}
	if v := args[n]; v.IsFun() {
...
func StrVal(s string) buzz.Value    { return buzz.StrValue(s) }
func IntVal(i int) buzz.Value       { return buzz.IntValue(int64(i)) }
func BoolVal(b bool) buzz.Value     { return buzz.BoolValue(b) }
func FloatVal(f float64) buzz.Value { return buzz.FloatValue(f) }

// StrSliceVal converts []string to a Buzz list.
func StrSliceVal(s []string) buzz.Value {
...
func StrMapVal(m map[string]string) buzz.Value {
	out := buzz.NewMap()
	for k, v := range m {
...
func AnyMapVal(m map[string]any) buzz.Value {
	out := buzz.NewMap()
	for k, v := range m {
...
func valToAny(v buzz.Value) any {
	switch {
	case v.IsBool():
		return v.AsBool()
	case v.IsInt():
...
func RecordsVal[T Recorder](rs []T) buzz.Value {
	items := make([]buzz.Value, len(rs))
	for i, r := range rs {
