...
// The docs are handwritten (the "Why/Fix" prose is the value), so this is the guard
// that keeps a new code from shipping without its page and that pins URL() routing to
// a real file.
func TestEveryDiagnosticCodeHasDocPage(t *testing.T) {
	...
	knownUndocumented := map[DiagnosticCode]bool{
		DaemonSocketWithheld:  true, // MGS2008
		NetEgress:             true, // MGS2009 (declared but not yet raised)
		SandboxPolicyMismatch: true, // MGS2010
	}
