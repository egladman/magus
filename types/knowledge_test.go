...
func TestEmptyAnswerAbsent(t *testing.T) {
	assert.Equal(t, KnowledgeAnswer{Verdict: VerdictAbsent}, EmptyAnswer(true, nil))
}
...
func TestEmptyAnswerIndexMissing(t *testing.T) {
	...
	assert.Equal(t, want, EmptyAnswer(true, gaps))
}
...
func TestEmptyAnswerSymbolsNotLoaded(t *testing.T) {
	want := KnowledgeAnswer{Verdict: VerdictUnknown, Reason: ReasonSymbolsNotLoaded}
	assert.Equal(t, want, EmptyAnswer(false, nil))
}

// Both causes at once. "not loaded" wins because it is the more specific fact and the
// more actionable fix: building the missing index would leave it just as unsearchable by
// this lookup, so reporting the index first would send the caller somewhere useless.
func TestEmptyAnswerNotLoadedWinsOverMissingIndex(t *testing.T) {
	gaps := []KnowledgeSymbolGap{{Project: NewProjectRef("libs/api", ""), State: SymbolIndexNotBuilt}}
	got := EmptyAnswer(false, gaps)
...
func TestKnowledgeAnswerJSONKeys(t *testing.T) {
	b, err := json.Marshal(EmptyAnswer(true, nil))
	require.NoError(t, err)
	assert.JSONEq(t, `{"verdict":"absent"}`, string(b))

	b, err = json.Marshal(EmptyAnswer(true, []KnowledgeSymbolGap{
