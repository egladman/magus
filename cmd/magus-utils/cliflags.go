...
func runCLIFlags(args []string) error {
	...
	b.WriteString("// Flag names declared by internal/cli's command registry, one constant\n")
...
func flagsForMode(flags []cli.Flag, mode string) []cli.Flag {
	var out []cli.Flag
	for _, f := range flags {
		if len(f.Modes) == 0 {
			if mode == "" {
...
func writeBinder(b *bytes.Buffer, command, constCommand string, flags []cli.Flag) {
	if len(flags) == 0 {
		return
	}
...
func goType(k cli.FlagKind) string {
	switch k {
	case cli.FlagBool:
		return "bool"
	case cli.FlagInt:
...
func bindMethod(k cli.FlagKind) string {
	switch k {
	case cli.FlagBool:
		return "Bool"
	case cli.FlagInt:
...
func flagDefaultLiteral(f cli.Flag) string {
	switch f.Kind {
	case cli.FlagBool:
