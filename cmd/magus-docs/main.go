...
func renderModule(m std.Module) string {
	...
	// Runnability note. WASM-compatible modules (dry.WASMCompatibleHostModules) get
	...
	if _, wasmCompatible := dry.WASMCompatibleHostModules[m.Name]; !wasmCompatible && moduleHasExample(m) {
		fmt.Fprintf(&b, "> [!NOTE]\n"+
	...
	if len(m.Methods) > 0 {
		...
		for _, meth := range m.Methods {
			...
			// WASM-compatible modules (dry.WASMCompatibleHostModules) get a
			...
			if ex := readExample(m.Name, meth.Name); ex != "" {
				...
				fmt.Fprintln(&b)
				if _, ok := dry.WASMCompatibleHostModules[m.Name]; ok {
					fmt.Fprintln(&b, "<!-- run -->")
				}
				fmt.Fprintln(&b, "```buzz")
				fmt.Fprint(&b, ex)
