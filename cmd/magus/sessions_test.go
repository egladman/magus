...
func TestRenderPromptCacheShowsEveryProvider(t *testing.T) {
	...
	renderPromptCache(&out, trail.AgentActivity{Session: "s1"}, sessions.PromptCache(last, now), now)
	text := out.String()
...
func TestPromptCacheHereReadsThisCheckoutsTrail(t *testing.T) {
	...
	_, _, ok := promptCacheHere(root, time.Now())
	assert.False(t, ok, "a checkout no agent has run against has no activity to clock")

	cacheDir, err := magus.ResolveCacheDir(root, magus.WithLoadedConfig(globalCfg))
	require.NoError(t, err)
	trail.AppendAgentCommand(t.Context(), cacheDir, trail.AgentCommand{Session: "s1", Tool: "shell.command", Command: "ls", Decision: "pass"})

	act, clock, ok := promptCacheHere(root, time.Now())
	require.True(t, ok)
	assert.Equal(t, "s1", act.Session)
