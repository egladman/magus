...
There is no free-text/`note` type.{{if .Full}} A claim that is true about the code is a
`pointer` of kind `query` (fetch it live) or `output`, never stored prose.{{else}} A claim true
about the code is a `query` or `output` pointer, never stored prose.{{end}}
...
- Use `put` for a decision or plan another person would otherwise have to
  rediscover.{{if .Full}} The CLI is often clearer for a human:{{end}}
