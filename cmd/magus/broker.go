...
func brokerServe(ctx context.Context, args []string) error {
	...
	rest, err := cmdParse("broker", args, func(fs *flag.FlagSet) {
		f = gen.BindBroker(fs, gen.BrokerDefaults{IdleExit: broker.DefaultIdleExit})
	...
	if len(rest) > 0 {
		return usagef("magus broker: takes no arguments (got %q)", rest[0])
	}
	if f.IdleExit < 0 {
	...
	fmt.Fprintf(os.Stderr, "magus: broker (pid %d, protocol %d) %s %s: %d slots, %s; %s\n",
		os.Getpid(), broker.ProtocolVersion, from, addr, slots, cache.FormatMB(memMB), exits)
...
func brokerStop(ctx context.Context, args []string) error {
	...
	// Waiting for the hangup rather than for the socket file to go: under systemd the
	// supervisor keeps the socket, and the next connection starts another broker.
