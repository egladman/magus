...
func TestCLICommandServerLeavesAreRealSubcommands(t *testing.T) {
	got := []string{hint.ServerStart.Leaf(), hint.ServerStop.Leaf(), hint.ServerReload.Leaf()}
	want := []string{"start", "stop", "reload"}
	if !slices.Equal(got, want) {
		t.Errorf("server leaves = %v, want %v", got, want)
	}
	for _, c := range []hint.Command{hint.ServerStart, hint.ServerStop, hint.ServerReload} {
		if c.Head() != "server" {
