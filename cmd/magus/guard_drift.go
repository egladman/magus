...
// hashed session id as every advisory marker, and so the retention sweep that runs
// beside those markers reaps it too.
const advisoryTouchedProjects advisoryKind = "touched-projects"
...
func (g advisoryGate) touchedProjects() []string {
	if g.base == "" {
		return nil
	}
	body, err := os.ReadFile(g.markerPath(advisoryTouchedProjects))
	if err != nil {
		return nil
	}
