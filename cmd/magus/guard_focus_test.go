...
func TestFocusReadOperands(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			cmds, ok := parseGuardCommands(tc.command)
			require.True(t, ok)
...
func TestFocusGradeSaysNothingWithoutAWorkspace(t *testing.T) {
	...
	assert.Empty(t, gradeFocusRead(ctx, "", "cat libs/ui/theme.css").Decision)
	assert.Empty(t, gradeFocusRead(ctx, "", "").Decision)
	assert.Empty(t, gradeFocusRead(ctx, "", "cat 'unterminated").Decision)
}
