...
func BindFlags(fs *flag.FlagSet, cfg *config.Config) {
	...
	fs.DurationVar(&cfg.Daemon.Maintenance.RotateActivities, "daemon-maintenance-rotate-activities", cfg.Daemon.Maintenance.RotateActivities, "MAGUS_DAEMON_MAINTENANCE_ROTATE_ACTIVITIES: RotateActivities is only how often the daemon CHECKS whether the activity trail is due for a trim; it is not a retent...")
	fs.DurationVar(&cfg.Daemon.Maintenance.RotateLogs, "daemon-maintenance-rotate-logs", cfg.Daemon.Maintenance.RotateLogs, "MAGUS_DAEMON_MAINTENANCE_ROTATE_LOGS: RotateLogs is both how often the daemon checks the run-log journals AND, since the check doubles as the enforcement, ...")
	fs.DurationVar(&cfg.Daemon.Maintenance.PrunePreserved, "daemon-maintenance-prune-preserved", cfg.Daemon.Maintenance.PrunePreserved, "MAGUS_DAEMON_MAINTENANCE_PRUNE_PRESERVED: PrunePreserved is only how often the daemon checks for expired 'vcs checkpoint --preserve' captures; it is not the re...")
