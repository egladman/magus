// Command magus-types-gen emits the Buzz `object` mirror of a Go value type in
// github.com/egladman/magus/types, so the Go struct stays the single source of
// truth and the two shapes can never drift. Run via `go generate`:
//
//	//go:generate go run ../../cmd/magus-types-gen -type TargetQuery -out buzzlib/targetquery.gen.buzz
//
// The emitted file is shipped in the magus/target module (see internal/spell), so
// `import "magus/target"` brings the type into a magusfile's or spell's scope for
// annotations and literals. Never linked into the binary.
package main
...
func main() {
	typeName := flag.String("type", "", "value type to mirror (e.g. TargetQuery)")
	outPath := flag.String("out", "", "output .buzz file path")
	flag.Parse()

	rt, ok := registry[*typeName]
	if !ok || *outPath == "" {
		fmt.Fprintln(os.Stderr, "usage: magus-types-gen -type <Name> -out <path.buzz>")
		os.Exit(2)
	}

	out, err := emit(*typeName, rt)
	if err != nil {
		fmt.Fprintf(os.Stderr, "magus-types-gen: %v\n", err)
		os.Exit(1)
	}
	if err := os.WriteFile(*outPath, out, 0o644); err != nil {
		fmt.Fprintf(os.Stderr, "magus-types-gen: write %s: %v\n", *outPath, err)
		os.Exit(1)
	}
}
...
func emit(name string, rt reflect.Type) ([]byte, error) {
	var b bytes.Buffer
	fmt.Fprintln(&b, "// Code generated by magus-types-gen. DO NOT EDIT.")
	fmt.Fprintln(&b, "//")
