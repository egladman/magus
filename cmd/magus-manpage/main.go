...
after the subcommand word. Last-write-wins, matching kubectl conventions.`
	...
	flagConcurrency = `Maximum number of concurrent build steps. 0 means use the configured value (or MAGUS_CONCURRENCY, or concurrency_profile, balanced (min(NumCPU,8)) by default).`
