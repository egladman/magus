...
**Magusfiles.** [Magusfile.bzz](#magusfilebzz) · [Runtime API](#runtime-api-three-tiers) · [Spells](#spells)
...
This writes a `magus.yaml` seeded with built-in defaults, stubs a starter `magusfile.bzz`, and wires the VCS merge driver so conflicts in declared outputs regenerate instead of leaving conflict markers. The VCS is auto-detected; pass `--vcs git|hg` to choose it in a non-interactive shell. By default the config is written to `$XDG_CONFIG_HOME/magus/magus.yaml`. Use `--local` to write it into the repo instead, or `--global` to write only the config (skip the workspace bootstrap).

**2. Edit the magusfile**

`magus init` drops a starter `magusfile.bzz` registering your project and wiring the CI pipeline. Targets are declared as exported functions — no registration call needed:

```buzz
// magusfile.bzz (generated by `magus init`)
import "magus";
...
#### Consuming a file spell (`magusfile.bzz`)
...
Editing a custom command in your `magusfile.bzz` (or any file under
...
If a `magusfile.bzz` is present at or above the current directory, it is executed automatically on startup so registered targets and locals are available.
...
```sh
magus run build
# *** magus.pry at magusfile.bzz:4 in build
