...
When the daemon is running it also exposes an **MCP server** over Streamable HTTP so agents and IDE plugins (Claude Desktop, Cursor, VS Code Copilot, …) can call magus directly. `magus server start` brings it up at `http://127.0.0.1:7391/mcp`; disable with `mcp.enabled: false` or `MAGUS_MCP_ENABLED=0`.
