...
and still catches breakage in a project you never opened. See [CI](docs/targets/ci.md).
...
and the graph. A [target](docs/targets.md) is a unit of work such as build,
test, or lint. A [spell](docs/spells.md) is a language adapter that supplies a
target's operations (the `go` spell provides `go-test`; the `buf` spell provides
`buf-lint`). A [charm](docs/charms.md) is a modifier applied to a run, like `rw`
for read-write or `cd` for a working directory. An [op](docs/operations.md) is a
...
to install. See the [Download guide](docs/download.md).
...
magus describe file website/gen/index.html   # is this file generated, and by what
```
...
- **Boundaries.** Each region is tagged with the Go package or project that owns it, so the diagram doubles as a code map: the runtime is the root module (`cmd/magus` plus `internal/*`), the browser apps are the [`website/`](https://github.com/egladman/magus/tree/main/website) project, and the wire contracts are the [`proto/magus`](proto/magus) protobufs.
...
magus is fully featured from the terminal, so everything here is optional. Alongside the CLI, the daemon can drive three read-only browser surfaces.

> **See it first:** [open the live demo](https://eli.gladman.cc/magus/console/) - no install, no daemon. It fills the dashboard with synthesized activity, streams a build into the log viewer, and lets you jump between all three apps in demo mode. Everything below runs against your own daemon instead.
