...
func TestWithDependsOnRelativeSibling(t *testing.T) {
	root := makeWorkspaceRoot(
		t,
		"magusfile.tl",                  // project "."
		"extensions/api/magusfile.tl",   // project "extensions/api"
		"extensions/drape/magusfile.tl", // project "extensions/drape"
	)
...
func TestWithSpellAddsLanguage(t *testing.T) {
	root := t.TempDir()
	if err := os.WriteFile(filepath.Join(root, "magusfile.tl"), []byte("//go:build magus\npackage main\n"), 0o644); err != nil {
		t.Fatal(err)
	}
