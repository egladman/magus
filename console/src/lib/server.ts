...
//   - HOST RESOLUTION: resolveServerHost() is the single source of truth for "which
...
// prompt describes. An error the server SENT (a 500, a Connect status) is not one of them, and
// telling that reader to start a server sends them to fix the wrong thing.
//
...
// wantsDemo reports whether the fragment requested the server-free demo. The
...
// Read it after adoptServerOrigin, or a server-origin page is not yet known to be attached.
export function mayLoadBundledDemo(params: HashParams = parseHash()): boolean {
  return serverAttach(params) === null;
}

// ---- the loopback lock -----------------------------------------------------

// validateLoopbackHost: a configured/entered server host MUST be literally 127.0.0.1
...
// here - that path resolves through resolveServerHost/location.host directly, so this
...
// console reaching a local server) can ONLY be 127.0.0.1 - a LAN IP is blocked as mixed
...
// normalizeServerHost accepts either a BARE PORT ("8787" -> "127.0.0.1:8787", the same
// literal-IP expansion #port= uses) or a full loopback host:port ("127.0.0.1:8787",
// "[::1]:8787"), returning the canonical "host:port" or null. The Settings server-address
// field and the connection probe run typed input through this so a user can enter just a
// port.
export function normalizeServerHost(input: string): string | null {
  const s = input.trim();
  if (s === "") return null;
...
// logsLink builds a log-viewer deep-link ("../logs/#...") for the server the console is
// talking to. A LOOPBACK server needs its port so the viewer can re-attach (#port=<port>);
// a LAN-share (same-origin LAN) server needs none - the viewer resolves its own origin -
...
// ownOrigin: the console adopted the page's OWN origin as the server (it was served BY
// the server and carries a token). Covers BOTH a device on a LAN share and the operator's
// own server-origin console. readOnly narrows that to the look-only case (a non-loopback LAN
...
// device loaded from the server's LAN share origin (see adoptServerOrigin). The shell uses
...
// adoptServerOrigin detects a page that must adopt its OWN origin as the server: a
// page carrying a #token= fragment (or a token already stashed from one) that was served
// BY the server. Two audiences reach it:
...
//     control.
//   - the operator's own console, opened by a minted server-origin link served from
//     127.0.0.1:<port>/console/ (LOOPBACK page origin): it adopts its origin as the server
//     too, but keeps FULL control - it is the operator's own console, not a shared viewer,
//     so it does NOT become read-only.
// It records the adoption (ownOrigin) so resolveServerHost returns location.host directly -
// no fragment synthesis - and consumes the token. A page carrying an explicit #port= is a
// cross-origin attach to a LOCAL server (the operator reaching past their origin), not
...
export function adoptServerOrigin(): boolean {
  if (typeof location === "undefined") return false;
  const params = parseHash();
  // An explicit #port attach reaches a loopback server from wherever the console is hosted:
  // full control, and the server is 127.0.0.1 (not this origin). Just consume any token.
...
//   2. own-origin adoption (a LAN-share viewer, or the operator's server-origin console) ->
...
export function serverAttach(params: HashParams = parseHash()): string | null {
...
export function resolveServerHost(params: HashParams = parseHash()): string | null {
  ...
  // normalizeServerHost (not validateLoopbackHost) so a stored bare port resolves the same way the
  // Settings field accepts one - "8787" expands to 127.0.0.1:8787 rather than reading as unset.
  return configured ? normalizeServerHost(configured) : null;
}
...
// probeServer answers "is anything listening at this host:port?" for the Settings test-connection
// control. /livez is the server's only tokenless route (health checks are mounted unguarded so a
// kubelet can reach them). Current servers also wrap /livez and /readyz in the same CORSAllow
...
export async function probeServer(hostPort: string, timeoutMs = 3000): Promise<ProbeResult> {
  const host = normalizeServerHost(hostPort);
  if (!host) {
    return {
      ok: false,
      reason:
  ...
  try {
    ...
    return {
      ok: false,
      reason:
        "Could not reach " + url + ". Is the server running? Start it with: magus server start",
    };
  }
}
...
// ReadinessComponent/ReadinessReport mirror the server's GET /readyz JSON body. Component names are
...
// from the network - a future server component or status value must not fail to typecheck against a
...
// fetchReadiness reads GET /readyz for the server's own component-level health (workspaces, symbol
// index, services, knowledge graph) - richer than probeServer's bare "did anything answer". Unlike
// /livez, /readyz answers WITH CORS headers and a JSON body on current servers, so this is a normal
...
//
// WHY graceful degradation: the server the caller is pointed at may PREDATE this endpoint's CORS/JSON
// support. Every failure mode here - network error, timeout, an old server's opaque/non-CORS response,
...
export async function fetchReadiness(
  ...
  if (!host) return null;
  // Defense in depth: the caller passes an already-resolved host (resolveServerHost), but re-verify
  ...
  try {
    ...
    // not-a-failure: an older server has no JSON /readyz; reachability itself is reported by the
    // server transport and fetchSSE. A timeout, a refused connection and a non-JSON body all
...
// the user toggles a "remember this server" control.
export function setRemembered(on: boolean): void {
  try {
    ...
    reportFailure(
      "Settings",
      "Could not " + (on ? "remember" : "forget") + " this server's token: " + errMessage(e),
      "token:remember",
    );
  }
}
...
// no token is stored (a server started without connector auth).
...
export function reportRpcFailure(host: string, service: string, method: string, e: unknown): void {
  ...
  if (isUnreachable(e)) {
    reportFailure(
      "Server",
      "Could not reach the server at " + host + ". Start it with `magus server start`.",
...
// reportFetchFailure is reportRpcFailure for a plain fetch to the server (the /api/v1 routes that
...
export function reportFetchFailure(host: string, what: string, e: unknown): void {
  ...
  reportFailure("Server", "Could not read " + what + ": " + errMessage(e), "fetch:" + what);
}

// reportHttpStatus reports a plain fetch the server answered with a non-2xx status. A 401 signs the
...
export function reportHttpStatus(host: string, what: string, status: number): void {
  ...
  reportFailure(
    "Server",
    "Could not read " + what + ": the server answered HTTP " + status + ".",
    "http:" + what + ":" + status,
  );
}

// Refusal is what a person needs from a server error body: its message, and the Help link naming
...
// createServerTransport points a browser-native Connect transport at the server
// origin, with the bearer interceptor pre-wired. Callers pass an already-resolved
// host (resolveServerHost/serverAttach) - never a raw fragment string.
...
// (console.OpenCommandAs is the server's twin of this line). The opener follows the browser's
// platform, which is the machine a loopback server runs on. Windows gets PowerShell's opener, since
