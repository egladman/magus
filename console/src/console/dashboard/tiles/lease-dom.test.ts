...
import { leaseTile } from "./lease";
import { initialState } from "../state";

function response(leases: unknown[]): Response {
  return {
    ok: true,
    status: 200,
    json: async () => ({ leases, overlaps: [] }),
  } as Response;
}
...
test("summarizes the active ledger on the dashboard", async () => {
  ...
  const tile = leaseTile();
  try {
    tile.update({ ...initialState(), liveHost: "daemon.test" });
    await new Promise((resolve) => setTimeout(resolve, 0));
    assert.equal(
      tile.el.querySelector(".console-dashboard-lease__summary")?.textContent,
      "3 leases. 1 declared, 1 running, 1 pass. 0 no-return.",
    );
    assert.equal(
      tile.el.querySelector(".console-dashboard-lease__summary")?.getAttribute("aria-live"),
      "polite",
    );
    assert.deepEqual(
      [...tile.el.querySelectorAll(".console-dashboard-lease__id")].map((el) => el.textContent),
      ["dashboard", "diff"],
    );
  } finally {
    tile.destroy();
