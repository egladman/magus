...
export function latencyTile(): Tile {
  ...
  const buildAll = () => {
    for (const k of LAT_KEYS) charts[k].build();
  };
  const resizeAll = () => {
    for (const k of LAT_KEYS) charts[k].resize();
  };

  const card = new Card("latency", "Latency", {
    term: "Latency",
  ...
  const note = h("span");
  note.append(
    h("span", "console-dashboard-legend console-dashboard-legend--p50", "p50"),
    h("span", "console-dashboard-legend console-dashboard-legend--p95", "p95"),
    h("span", "console-dashboard-legend console-dashboard-legend--p99", "p99"),
  ...
  const offTheme = onThemeChange(() => {
    for (const k of LAT_KEYS) charts[k].rebuild();
  });
