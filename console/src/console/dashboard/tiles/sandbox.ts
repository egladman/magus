...
export function sandboxTile(): Tile {
  const card = new Card("sandbox", "Filesystem sandbox", {
    term: "Sandbox",
    defaultCollapsed: true,
    note: "rules and access checks",
  });
  const grid = new MetricGrid([
    ...
    },
    {
      caption: "Rules",
      items: [
        { key: "rulesRead", label: "read" },
        { key: "rulesWrite", label: "write" },
        { key: "rulesExec", label: "exec" },
        { key: "envRules", label: "env rules" },
      ],
    },
    {
      caption: "Checks",
      items: [
        { key: "checksAllow", label: "allow" },
        { key: "checksDeny", label: "deny" },
        { key: "envDropped", label: "env dropped" },
      ],
    },
  ]);
  ...
  function render(sb: SandboxView): void {
    ...
    grid.set("envRules", fmtCount(sb.envRules));
    grid.set("checksAllow", fmtCount(sb.checksAllow));
    grid.set("checksDeny", fmtCount(sb.checksDeny));
