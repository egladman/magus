...
export function demoLeases(nowMs: number): Lease[] {
  ...
  return [
    ...
    // on, which is what the release digest records.
    {
      ...
      write_paths: ["libs/authkit/claims.go", "libs/authkit/audience.go"],
      deny_paths: ["services/**", "apps/**"],
      created: at(104),
      updated: at(88),
      releases: [
        {
    ...
    // The Go reader. Green, and it waited on the type above.
    {
      ...
      write_paths: ["services/identity/internal/token/"],
      created: at(96),
      updated: at(80),
    },
    ...
    // and the reader decides whether the plan meant it.
    {
      ...
      write_paths: ["services/identity/internal/token/verify_test.go"],
      created: at(96),
      updated: at(58),
    },
    ...
    // The stale badge belongs to docs-rename below, which is non-terminal and untouched.
    {
      ...
      depends_on: ["authkit-type"],
      write_paths: ["apps/dashboard/src/api/session.ts"],
      created: at(96),
      updated: at(84),
    },
