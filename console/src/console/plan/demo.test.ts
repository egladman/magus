...
import { buildPlan, isTerminal, normalizeState, UNIT_STATES, type UnitState } from "./ledger";
...
test("the demo ledger builds a plan with every unit rooted in one tree", () => {
  const model = buildPlan(demoUnits(NOW), demoOverlaps());
...
test("every state the surface can draw appears in the demo", () => {
  const seen = new Set<UnitState>(demoUnits(NOW).map((u) => normalizeState(u.state)));
  for (const s of UNIT_STATES) {
    assert.ok(seen.has(s), `the showcase must exercise the ${s} state`);
  }
});

test("the overlap names two units that exist and really do intersect", () => {
  const ids = new Set(demoUnits(NOW).map((u) => u.id));
  const overlaps = demoOverlaps();
  assert.ok(overlaps.length > 0, "the overlap warning must be reachable in the demo");
  for (const o of overlaps) {
    assert.ok(ids.has(o.unit_a), `${o.unit_a} is not a unit in the demo`);
    assert.ok(ids.has(o.unit_b), `${o.unit_b} is not a unit in the demo`);
    ...
    assert.ok(hit, `${o.unit_a} and ${o.unit_b} are declared to overlap but their paths do not`);
  }
});

test("a stale non-terminal unit exists, since a terminal row draws no warning", () => {
...
test("dependencies and parents only ever name units that exist", () => {
  const units = demoUnits(NOW);
  const ids = new Set(units.map((u) => u.id));
  for (const u of units) {
    if (u.parent) assert.ok(ids.has(u.parent), `${u.id} names a missing parent ${u.parent}`);
    for (const d of u.depends_on ?? []) {
      assert.ok(ids.has(d), `${u.id} depends on a missing unit ${d}`);
    }
  }
});
...
test("the fixture is a pure function of now, so two reads agree", () => {
  assert.deepEqual(demoUnits(NOW), demoUnits(NOW));
