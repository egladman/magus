...
export function scenarioRuns(now: number): ScenarioRun[] {
  const at = (endMin: number, durMs: number): { startMs: number; endMs: number; durationMs: number } => {
    const endMs = now - endMin * MIN;
    return { startMs: endMs - durMs, endMs, durationMs: durMs };
  };
  return [
    {
      ref: "out5b0091", inv: INV_BUILD_FRESH, project: "svc/api", target: "build", trigger: "mcp",
      state: "passed", ...at(3, 2100),
      execs: ["go build ./cmd/api"], stdout: "ok  svc/api:build  2.1s",
    },
    {
      ref: "out40de1a", inv: "inv40de", project: "svc/api", target: "generate", trigger: "cli",
      state: "failed", ...at(40, 300),
    ...
    {
      ref: "out80f2c7", inv: INV_TEST_FIX, project: "svc/api", target: "test", trigger: "mcp",
      state: "passed", ...at(80, 3600),
      execs: ["go test ./users"], stdout: "ok  svc/api:test  3.6s",
    },
    {
      ref: "out84a3b0", inv: "inv84a3", project: "web/app", target: "typecheck", trigger: "cli",
      state: "passed", ...at(84, 2400),
      execs: ["tsc --noEmit"], stdout: "ok  web/app:typecheck  2.4s",
    },
    {
      ref: "out88c110", inv: "inv88c1", project: "lib/core", target: "lint", trigger: "cli",
      state: "passed", ...at(88, 640),
      execs: ["golangci-lint run ./..."], stdout: "ok  lib/core:lint  0.6s",
    },
    {
      ref: "out92e1d4", inv: INV_TEST_BREAK, project: "svc/api", target: "test", trigger: "mcp",
      state: "failed", ...at(92, 4200),
      error: "assertion failed: users_test.go:118: got 500, want 200",
      execs: ["go build ./...", "go test ./users"],
      stderr: "--- FAIL: TestListUsers (0.03s)\n    users_test.go:118: got 500, want 200\nFAIL svc/api:test",
    },
    // The `magus affected ci` sweep ~3h ago: one invocation, mostly cache hits, all green.
    {
      ref: "outci7a1", inv: INV_CI, project: "svc/api", target: "test", trigger: "ci",
      state: "passed", ...at(178, 3900),
      execs: ["go test ./..."], stdout: "ok  svc/api:test  3.9s",
    },
    {
      ref: "outci7a2", inv: INV_CI, project: "web/app", target: "build", trigger: "ci",
      state: "cached", ...at(179, 180),
      stdout: "cached  web/app:build",
    },
    {
      ref: "outci7a3", inv: INV_CI, project: "lib/core", target: "build", trigger: "ci",
      state: "cached", ...at(179, 95),
      stdout: "cached  lib/core:build",
    },
    {
      ref: "outci7a4", inv: INV_CI, project: ".", target: "lint", trigger: "ci",
      state: "passed", ...at(180, 610),
      execs: ["golangci-lint run ./..."], stdout: "ok  .:lint  0.6s",
    },
  ];
}
...
export function scenarioActivity(now: number): ScenarioActivity[] {
  ...
  const run = (inv: string): ScenarioRun => runs.find((x) => x.inv === inv)!;
  ...
  return [
    {
      kind: "mcp", actor: "claude-code", action: "magus_run_target", ok: true,
      timeMs: fresh.startMs, durationMs: fresh.durationMs,
      requestBytes: 198, requestRef: "reqb5d091", responseBytes: 4100, responseRef: fresh.ref,
      preview: "ok  svc/api:build  2.1s", workspace: WORKSPACE,
    },
    {
      kind: "mcp", actor: "claude-code", action: "magus_output", ok: false,
      timeMs: at(18), durationMs: 6, requestBytes: 44,
      error: "unknown output reference: " + PRUNED_REF, workspace: WORKSPACE,
    },
    {
      kind: "sandbox", actor: "svc/api:generate", action: "write /etc/hosts", ok: false,
      timeMs: at(40),
      error: "sandbox denied a write outside the workspace: /etc/hosts", workspace: WORKSPACE,
    },
    {
      kind: "mcp", actor: "claude-code", action: "magus_run_target", ok: true,
      timeMs: fix.startMs, durationMs: fix.durationMs,
      requestBytes: 210, requestRef: "req80f2c7", responseBytes: 5200, responseRef: fix.ref,
      preview: "ok  svc/api:test  3.6s\n--- PASS: TestListUsers", workspace: WORKSPACE,
    },
    {
      kind: "mcp", actor: "claude-code", action: "magus_run_target", ok: false,
      timeMs: broke.startMs, durationMs: broke.durationMs,
      requestBytes: 210, responseBytes: 3800, responseRef: broke.ref,
      error: "svc/api:test failed: 1 assertion",
      preview: "FAIL svc/api:test\nusers_test.go:118: got 500, want 200", workspace: WORKSPACE,
    },
    {
      kind: "job", actor: "daemon", action: "symbol-reindex", ok: true,
      timeMs: at(96), durationMs: 1260,
      preview: "reindexed after branch switch: 1648 symbols across 42 files", workspace: WORKSPACE,
    },
    {
      kind: "mcp", actor: "claude-code", action: "magus_query", ok: true,
      timeMs: at(100), durationMs: 34,
      requestBytes: 88, responseBytes: 1640, responseRef: "outq100a",
      preview: "kind:target project:svc/api  ->  9 matches", workspace: WORKSPACE,
    },
    {
      kind: "config", actor: "eli", action: "mcp connector add", ok: true,
      timeMs: at(150),
      preview: "added connector token \"laptop\" (expires in 30d)", workspace: WORKSPACE,
    },
    {
      kind: "token", actor: "daemon", action: "cli token issued", ok: true,
      timeMs: at(155), workspace: WORKSPACE,
    },
  ];
}
...
export interface ScenarioInsight {
  commits: number;
  hotspots: { name: string; churn: number; authors: number; blastRadius: number; lastCommitMs: number }[];
  affinity: { a: string; b: string; count: number; hidden: boolean }[];
  ownership: { path: string; primary: string; primaryShare: number; authors: number; busFactor1: boolean; stale: boolean }[];
  trend: { path: string; delta: number; recent: number; earlier: number }[];
  volatility: {
    threshold: number;
    targets: { project: string; target: string; score: number; volatile: boolean; pass: number; fail: number; volatileCount: number; samples: number; lastPassMs: number }[];
  };
}
...
export function scenarioInsight(now: number): ScenarioInsight {
  ...
  return {
    ...
    hotspots: [
      ...
      { name: "svc/api/users_test.go", churn: 24, authors: 2, blastRadius: 9, lastCommitMs: at(80) },
      { name: "web/app/src/api/client.ts", churn: 19, authors: 2, blastRadius: 14, lastCommitMs: at(180) },
    ],
    affinity: [
    ...
    ownership: [
      { path: "lib/core", primary: "eli", primaryShare: 91, authors: 2, busFactor1: true, stale: false },
      { path: "svc/api", primary: "eli", primaryShare: 74, authors: 3, busFactor1: false, stale: false },
      { path: "web/app", primary: "eli", primaryShare: 83, authors: 2, busFactor1: false, stale: true },
    ],
    trend: [
    ...
    volatility: {
      threshold: 0.2,
      targets: [
        { project: "svc/api", target: "test", score: 0.31, volatile: true, pass: 44, fail: 3, volatileCount: 4, samples: 47, lastPassMs: at(80) },
        { project: "web/app", target: "build", score: 0.06, volatile: false, pass: 58, fail: 0, volatileCount: 1, samples: 59, lastPassMs: at(179) },
      ],
    },
  };
}
