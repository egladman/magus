...
func (m *Magus) buildStep(p *types.Project, target string) cache.Step {
	...
	for _, proj := range m.All() {
		if proj.Path != p.Path && (p.Path == "." || strings.HasPrefix(proj.Path, p.Path+"/")) {
			step.NestedDirs = append(step.NestedDirs, proj.Path)
		}
...
func outputGlobsByRoot(ws *types.Workspace, p *types.Project, target string) []diff.OutputGlobs {
	...
	for root, globs := range byRoot {
		slices.Sort(globs)
		sets = append(sets, diff.OutputGlobs{Root: root, Globs: slices.Compact(globs), Nested: ws.NestedDirs(root)})
	}
...
func (m *Magus) executeStages(ctx context.Context, stages []stage, scopeLabel string, opts run) (err error) {
	...
	// runTarget opens the descendant audit around a step's body, which a hit never runs.
	cacheOpts = append(cacheOpts, cache.AuditReplay(func(ctx context.Context, s cache.Step, written []string) error {
...
func (m *Magus) gateDrift(ctx context.Context, p *types.Project, target string, policy types.DriftPolicy, fn func() error) error {
	...
	globs := []diff.OutputGlobs{{Root: dir, Globs: p.AllOutputs(), Nested: m.ws.NestedDirs(dir)}}
