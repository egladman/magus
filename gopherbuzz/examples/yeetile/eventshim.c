// eventshim.c — yeetile's event-tap helper, the one piece that can't be pure
...
// The four modifier bits yeetile matches on (shift, ctrl, alt, cmd). These all
// live in the low 21 bits of CGEventFlags, so a 32-bit int carries everything
// yeetile cares about — and i32 maps to Buzz `int` on both runtimes (i64 would
