...
# The event-tap helper (see eventshim.c) — the one bit of C yeetile needs so
# stock upstream can drive global hotkeys / the launcher key-grab without an
# FFI callback. zdef("yeetileshim", …) finds ./libyeetileshim.dylib.
cc -shared -o libyeetileshim.dylib eventshim.c -framework CoreGraphics -framework CoreFoundation -framework ApplicationServices -lobjc
echo "built libyeetileshim.dylib (event-tap helper)"
