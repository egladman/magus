...
func makeBufferValue(st *bufferState) buzz.Value {
	buf := &st.bytes
	m := mod()
	...
	m.MapSet("len", fn("Buffer.len", func(_ context.Context, args []buzz.Value) (buzz.Value, error) {
		align := 1
		...
		return buzz.IntValue(int64(n / align)), nil
	}))
	...
	m.MapSet("ptr", fn("Buffer.ptr", func(_ context.Context, args []buzz.Value) (buzz.Value, error) {
		at := 0
		if len(args) >= 1 && args[0].IsInt() {
			at = int(args[0].AsInt())
		}
		...
		if at < 0 || at > st.cap {
			return buzz.Null, fmt.Errorf("Buffer.ptr: offset %d out of range [0, %d]", at, st.cap)
		}
		...
		return buzz.IntValue(int64(addr) + int64(at)), nil
	}))
	...
	m.MapSet("writeZAt", fn("Buffer.writeZAt", func(_ context.Context, args []buzz.Value) (buzz.Value, error) {
		if len(args) < 3 || !args[0].IsInt() || !args[1].IsStr() || !args[2].IsList() {
			return buzz.Null, fmt.Errorf("Buffer.writeZAt: requires (int at, str zigType, [any] values)")
		}
		at := int(args[0].AsInt())
		zigType := args[1].AsString()
		size, _, ok := buzz.CTypeLayout(zigType)
		if !ok {
			return buzz.Null, fmt.Errorf("Buffer.writeZAt: unknown type %q", zigType)
		}
		addr, err := st.ensurePinned()
		if err != nil {
			return buzz.Null, err
		...
		for idx, v := range args[2].ListItems() {
			...
			case !isFloat && v.IsInt():
				i = v.AsInt()
			default:
				return buzz.Null, fmt.Errorf("Buffer.writeZAt: value %d for %q must be %s", idx, zigType, numKind(isFloat))
			}
	...
	m.MapSet("readZAt", fn("Buffer.readZAt", func(_ context.Context, args []buzz.Value) (buzz.Value, error) {
		if len(args) < 2 || !args[0].IsInt() || !args[1].IsStr() {
			return buzz.Null, fmt.Errorf("Buffer.readZAt: requires (int at, str zigType)")
		}
		addr, err := st.ensurePinned()
		if err != nil {
			return buzz.Null, err
		...
		size, _, ok := buzz.CTypeLayout(zigType)
		if !ok {
			return buzz.Null, fmt.Errorf("Buffer.readZAt: unknown type %q", zigType)
		}
