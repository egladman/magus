...
const __r = f();`); err != nil {
	...
	t.Run("annotated decl from any is checked", func(t *testing.T) {
		sess := newSession(ctx)
		err := sess.Exec(ctx, `var a = "x"; var u: any = a; var n: int = u; const __r = n;`)
...
func TestCheckTypeNoFalsePositives(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"int loop":          {`var s = 0; var i = 0; while (i < 5) { s = s + i; i = i + 1; } const __r = s;`, 10},
		"typed int decl":    {`var x: int = 7; const __r = x + 1;`, 8},
		"int from typed fn": {`fun id(n: int) int { return n; } var x: int = id(5); const __r = x + 1;`, 6},
		"reassign int":      {`var x = 1; x = 2; x = x + 3; const __r = x;`, 5},
	}
