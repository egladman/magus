...
| [`go`](spells/go.md) | Go | 9 | Go toolchain spell: build, test, vet, fmt, mod-tidy, golangci-lint, and govulncheck as magus ops. |
| [`md`](spells/md.md) | Markdown | 3 | Markdown docs spell: markdownlint and prettier for linting and formatting prose. |
| [`py`](spells/py.md) | Python | 5 | Python toolchain spell: pytest, ruff check/format, and uv build/clean as magus ops. |
| [`rs`](spells/rs.md) | Rust | 5 | Rust toolchain spell: cargo build, test, clippy, fmt, and clean as magus ops. |
| [`ts`](spells/ts.md) | TypeScript | 5 | TypeScript toolchain spell: tsc, eslint, prettier, and vitest run through the project package manager. |
<!-- END SPELL LIST -->
...
- A **service op** returns a `Service` — a long-running process. `command` (required) is the process, which `magus run <target>` forks in the **foreground and blocks** on (Ctrl-C signals it), rather than running to completion. `readiness` (a probe polled until it exits 0) and `stop` (a graceful-shutdown command) are **optional**; they are consumed once magus supervises processes in the background, so today only `command` runs. (`Service` is a distinct return type so the op's kind is inferred from what it returns.) A service op's target is **inherently uncached** — magus never replays or snapshots it, so a re-run restarts the process instead of a cache hit doing nothing.
...
| **Op**             | A command op forks a `Command` (`{bin, args, charms}`) to completion; a service op is a long-running `Service` (`{command, readiness?, stop?}`) `magus run` blocks on.                                                   |
| **Op kind**        | Whether an op is a `command` (returns a `Command`, run to completion — the default) or a `service` (returns a `Service`, a long-running process `magus run` blocks on). Inferred from the return type; lives on the op, so one spell mixes both. |
