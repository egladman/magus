#!/usr/bin/env bash
# queue.sh: merge-queue scenarios on the enriched fixture, driving `magus queue` with the
# fixture's own magus workspace answering what each change affects.
#
#   Q1  planning latency over N open changes, each editing one feature library:
#       admission, conflict checks, the affected set of every change (the ci target),
#       and partitioning.
#   Q2  speculative stages: three stacked changes to the platform packages (logging,
#       then http, then metrics: one partition), validated by `magus queue validate`
...
#       from the verdicts.
#   Q3  apply as each stage goes green: validation at --depth 3 and `magus queue apply`
...
BASE="bench/queue"
PROVIDER="$DIR/queue-local.buzz"

[[ -d "$REPO/.git" ]] || { echo "queue: run ./setup.sh first (BENCH_SKIP_INSTALL=1 is enough)" >&2; exit 1; }
MAGUS="$(command -v "$MAGUS")"
mkdir -p "$OUT"
# The fixture is its own remote: the queue fetches the base and every change from it.
Q=("$MAGUS" -C "$REPO" queue)
...
for n in $SIZES; do
    ...
    hyperfine --runs "$RUNS" --warmup 1 --export-json "$OUT/q1-$n.json" \
        "'$MAGUS' -C '$REPO' queue plan --remote . --provider '$PROVIDER' --changes '$OUT/q1-$n-changes.json' --out '$OUT/q1-$n-plan.json'" >/dev/null
    printf 'Q1 n=%-4s mean %7.3fs  partitions %s\n' "$n" \
        "$(jq '.results[0].mean' "$OUT/q1-$n.json")" "$(jq '.partitions | length' "$OUT/q1-$n-plan.json")"
done
...
platform_changes() {
    ...
    "${Q[@]}" plan --remote . --provider "$PROVIDER" --changes "$OUT/q2-changes.json" \
...
platform_changes 3
"${Q[@]}" validate --remote . --stdin --gate "$GATE" --verdicts "$OUT/q2-warmup" \
    < "$OUT/q2-plan-d3.json" > "$OUT/q2-warmup.jsonl" 2> "$OUT/q2-warmup.log"
for depth in 1 3; do
    rm -rf "$REPO/.magus/cache" "$OUT/q2-d$depth"
    platform_changes "$depth"
    start=$(now)
    "${Q[@]}" validate --remote . --stdin --gate "$GATE" --verdicts "$OUT/q2-d$depth" \
        < "$OUT/q2-plan-d$depth.json" > "$OUT/q2-d$depth.jsonl" 2> "$OUT/q2-d$depth.log"
    end=$(now)
...
start=$(now)
"${Q[@]}" validate --remote . --stdin --gate "$GATE" --verdicts "$OUT/q3" \
    < "$OUT/q3-plan.json" > "$OUT/q3-validate.jsonl" 2> "$OUT/q3-validate.log" &
validator=$!
QUEUE_REPO="$REPO" QUEUE_BASE="$apply_base" "${Q[@]}" apply --remote . --base "$apply_base" \
    --provider "$PROVIDER" --interval 200ms "$OUT/q3" > "$OUT/q3-apply.jsonl" 2> "$OUT/q3-apply.log"
wait "$validator"
