...
| `rename-logger-factory`     | generated     | $4.00 / 80 turns | `createLogger` appears nowhere in the tree, a held-out test that `makeLogger` and every platform caller and bridge still work, the packages' own tests, and `gen-api.mjs --check` | the name is still there in six files, so the grep gate trips first                                          | renaming across the tree and regenerating the summaries clears all four gates |
