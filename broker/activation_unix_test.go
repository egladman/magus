...
func TestAdoptListenerTakesAListeningUnixSocket(t *testing.T) {
...
func TestAdoptListenerRefusesWhatIsNotAListeningUnixStream(t *testing.T) {
	dir := t.TempDir()
...
// TestASocketActivatedBrokerServesTheSupervisorsSocket runs the whole handover the way
// systemd does it: the socket is bound before the broker exists, arrives as descriptor
// 3 with LISTEN_FDS=1, and the broker serves it without binding anything itself.
func TestASocketActivatedBrokerServesTheSupervisorsSocket(t *testing.T) {
	addr := testAddr(t)
