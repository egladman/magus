...
// run dials) is an error, never a reason to bind the socket itself.
//
...
// not make it. Under launchd the broker runs from a KeepAlive agent and binds its own
...
func listenFDs(lookup func(string) (string, bool), pid int) (int, error) {
	...
	if names, ok := lookup(envListenFDNames); ok && strings.Count(names, ":") != 0 {
