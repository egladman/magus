...
func TestAnalyzer(t *testing.T) {
	analysistest.Run(t, analysistest.TestData(), orphanator.Analyzer, "sprawl")
}
...
func TestAnalyzerUnpaired(t *testing.T) {
	analyzer, err := orphanator.New(orphanator.Options{Unpaired: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAnalyzerAllow(t *testing.T) {
	analyzer, err := orphanator.New(orphanator.Options{Allow: []string{"*_conformance_test.go"}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewRejectsMalformedGlob(t *testing.T) {
	_, err := orphanator.New(orphanator.Options{Allow: []string{"*_ok_test.go", "[bad"}})
	if err == nil {
