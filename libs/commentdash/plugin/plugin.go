// Package plugin adapts commentdash to golangci-lint's module plugin system.
//
// Separate from the analyzer so that commentdash itself carries no golangci-lint
...
import (
	"github.com/egladman/magus/libs/commentdash"
	"github.com/golangci/plugin-module-register/register"
	"golang.org/x/tools/go/analysis"
)

func init() {
	register.Plugin("commentdash", newPlugin)
}

// newPlugin builds the plugin from its linters.settings.custom.commentdash.settings
// block. It decodes into [commentdash.Options] directly rather than into a copy
...
func newPlugin(raw any) (register.LinterPlugin, error) {
	opts, err := register.DecodeSettings[commentdash.Options](raw)
	if err != nil {
		return nil, err
	}

	analyzer, err := commentdash.New(opts)
	if err != nil {
		return nil, err
	}
