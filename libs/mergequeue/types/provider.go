...
type ListQuery struct {
	...
	// provider's default credential. AppID is that app's integration id as the person
	// read it, for a provider that cannot read the app itself (github: a private app is
	// hidden from every token but its installation's). SetupSteps also asks for the
