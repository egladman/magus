...
func (r *applyRun) proveOwed(ctx context.Context, c types.Change, owed []regenerationProof) error {
	for _, ob := range owed {
		...
		got, err := r.regenerateAt(ctx, c, from, ob.Onto, ob.Paths, added, g.Units)
		if err != nil {
			return err
		}
...
func (r *applyRun) regenerateAt(ctx context.Context, c types.Change, commit, onto string, paths, changed, units []string) (string, error) {
	cand, err := checkout(ctx, r.vcs, r.clone.Root, r.scratch, "proof-"+c.ID, commit)
	if err != nil {
		return "", err
	}
	defer r.discard(ctx, cand)
	b := built{Candidate: cand, touched: paths, changed: changed}
	s := candidateSpec{clone: r.clone, facts: r.facts, onto: onto, change: c, scratch: r.scratch}
