...
type Location struct {
	...
	// REPOSITORY owns the rows (through internal/repo, which folds every worktree and clone of
...
func jobsPath(loc Location) (string, error) {
	...
	dir, err := repo.StateDir(base, "jobs", loc.Root)
	if err != nil {
	...
	if loc.CacheDir != "" {
		if err := repo.Adopt(filepath.Join(loc.CacheDir, "ledger"), dir); err != nil {
			return "", fmt.Errorf("job: %w", err)
		}
	}
...
// has none of its own.
//
// A COPY rather than a move, which is the whole reason this is not repo.Adopt: binaries
