...
var Registry = []ToolDescriptor{
	...
	},
	{
		...
		Description: "Record the orchestrating agent's declared delegation plan so humans can see it; the ledger itself refuses nothing. One row per delegation, in the magus-delegate-multi-agent vocabulary: goal and acceptance criteria, the checkpoint the delegation was handed, owned and forbidden paths, dependencies, tier, validation, and state. Owned/forbidden paths are a DECLARATION this store never acts on - the agent guard is what reads these facts to grade an agent's file writes, loudly and with the owning delegation named, which is a separate surface on purpose: a store that quietly enforced would teach agents to route around the ledger. Every row should end in pass, fail, or no_return; a read-only delegation carries an abbreviated row with no paths. One plan per workspace: clear starts a fresh one and keeps no history. Re-put your row on every state change: each put re-stamps updated, and a row nobody touches goes stale, so an orchestrator reading that staleness will treat the delegation as possibly dead - which is the READER's judgment, since nothing here transitions a row on its own.",
		Params: []ParamDescriptor{
			{Name: "op", Type: "string", Description: "One of: list (default; every row, plus overlaps - the pairs of live delegations whose owned_paths intersect, reported as unit_a/unit_b with each side's own declarations in paths_a/paths_b, derived on the read and stored nowhere, and a fact to look at rather than a verdict), put (create or replace one row by id), register (record reported_base, the base a worker actually landed on, and get the divergence verdict back), clear (drop every row to start a fresh plan)."},
			{Name: "reported_base", Type: "string", Description: "register only, REQUIRED: the checkpoint token the worker actually landed on, as `magus vcs checkpoint -o name` prints it. Recorded on the row under this same name, next to the checkpoint the delegation was handed. The answer is a verdict (match, revision-match, diverged, unknown) and a reading of it - a fact returned and stored, never a refusal."},
			{Name: "id", Type: "string", Description: "put and register, REQUIRED: the delegation's id, which put upserts on. Use the same id you put in the worker's prompt."},
			{Name: "parent", Type: "string", Description: "put only: the id of the delegation that delegated this one. Omit for a delegation the root spawned."},
			{Name: "goal", Type: "string", Description: "put only: the delegation's goal and its observable acceptance criteria (named tests, artifacts, diagnostics, review checks - not \"works correctly\")."},
			{Name: "checkpoint", Type: "string", Description: "put only: the working state this delegation was handed, as `magus vcs checkpoint -o name` prints it (revision, plus a dirty-patch digest when the tree was not clean)."},
			{Name: "owned_paths", Type: "string", Description: "put only: space-separated paths the delegation may edit. Empty on a read-only delegation by design. Shrinking this set IS how a delegation announces it has finished editing a path: the dropped paths are recorded on the row as releases, each with the sha256 of the file at that moment (absent when nothing is there, dir for a directory, unreadable when something is there that could not be hashed - which is deliberately not the same answer as absent), so the next agent knows WHICH version it inherits - a digest that no longer matches at verification time means it built on a tree the releaser never saw."},
			{Name: "forbidden_paths", Type: "string", Description: "put only: space-separated paths the delegation must not touch. Empty on a read-only delegation by design."},
			{Name: "depends_on", Type: "string", Description: "put only: space-separated ids of delegations that must land before this one."},
			{Name: "tier", Type: "string", Description: "put only: the effort tier the work was matched to, e.g. principal, standard, economy."},
			{Name: "validation", Type: "string", Description: "put only: the magus target or named check this delegation was assigned."},
			...
			{Name: "read_only", Type: "boolean", Description: "put only: mark a delegation that gathers evidence and writes nothing, so empty owned/forbidden paths read as deliberate rather than forgotten."},
		},
	},
}
