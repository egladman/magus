...
var Registry = []ToolDescriptor{
	...
	},
	{
		...
		Description: "Record the orchestrating agent's declared delegation plan so humans can see it; the ledger itself refuses nothing. One row per delegated unit, in the magus-delegate-multi-agent vocabulary: goal and acceptance criteria, the checkpoint the unit was handed, owned and forbidden paths, dependencies, tier, validation, and state. Owned/forbidden paths are a DECLARATION this store never acts on - the agent guard is what reads these facts to grade an agent's file writes, loudly and with the owning unit named, which is a separate surface on purpose: a store that quietly enforced would teach agents to route around the ledger. Every row should end in pass, fail, or no_return; a read-only unit carries an abbreviated row with no paths. One plan per workspace: clear starts a fresh one and keeps no history. Re-put your row on every state change: each put re-stamps updated, and a row nobody touches goes stale, so an orchestrator reading that staleness will treat the unit as possibly dead - which is the READER's judgment, since nothing here transitions a row on its own.",
		Params: []ParamDescriptor{
			{Name: "op", Type: "string", Description: "One of: list (default; every row, plus overlaps - the pairs of live units whose owned_paths intersect, reported as unit_a/unit_b with each side's own declarations in paths_a/paths_b, derived on the read and stored nowhere, and a fact to look at rather than a verdict), put (create or replace one row by id), register (record the base a worker actually landed on and get the divergence verdict back), clear (drop every row to start a fresh plan)."},
			{Name: "base", Type: "string", Description: "register only, REQUIRED: the checkpoint token the worker actually landed on, as `magus vcs checkpoint -o name` prints it. The answer is a verdict (match, revision-match, diverged, unknown) and a reading of it - a fact returned and stored, never a refusal."},
			{Name: "id", Type: "string", Description: "put and register, REQUIRED: the unit's id, which put upserts on. Use the same id you put in the worker's prompt."},
