...
// The unit is stamped from the SessionStart the caller supplied, never re-read per fact:
// a mid-run environment change would otherwise split one session's facts across two
// units, which is a history no producer could have meant.
func TestFactHandlerStampsTheSuppliedUnitOnEveryFact(t *testing.T) {
	t.Setenv("XDG_STATE_HOME", t.TempDir())
	root := t.TempDir()

	h := NewFactHandler(root, SessionStart{Workspace: root, Command: "run ci", Unit: "fleet/f3"})
	require.NotNil(t, h)
	emitFact(t, h, journal.Event{Kind: journal.KindResult, Inv: "inv1", Target: "build", Status: journal.StatusPass})
	emitFact(t, h, journal.Event{Kind: journal.KindResult, Inv: "inv1", Target: "test", Status: journal.StatusPass})
