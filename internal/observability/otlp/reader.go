...
// It exists so a caller (the server's dashboard aggregation) can read raw OTel
...
// fat Provider interface: only the local-collect server path needs it, so it is reached by
