...
-o json prints the mergequeue.capabilities/v1 document with the setup inside it.`,
		...
		},
		{
			...
			Flags: append(append([]Flag{
				{Name: "provider", Kind: FlagString, Doc: "`provider`: a built-in name (github) or a .buzz file"},
				{Name: "base", Kind: FlagString, Doc: "`branch` the queue merges into; a plan naming another is refused (MGS3025), and a run: source must have run on it"},
				{Name: "workflow", Kind: FlagString, Doc: "`definition` a run: source must have run, started by an event that runs the base's own copy of it (github: .github/workflows/queue.yaml); required with a run: source, whose uploads are otherwise refused (MGS3024)"},
