...
The wire is versioned: a client offers the protocol versions it speaks, the
broker picks the newest both share and refuses a client it shares none with.
A new broker keeps answering the previous version for at least one release.`,
	Usage: "magus broker [status|stop|units] [flags]",
	Flags: []Flag{
		{Name: "idle-exit", Kind: FlagDuration, Default: 10 * time.Minute, DefaultAtBind: true, Doc: "Exit once the broker has held nothing this long; 0 never exits, for a supervisor that keeps it alive"},
	},
	Children: []Command{
		{
			Name:        "status",
