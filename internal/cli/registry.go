...
-o json prints the mergequeue.capabilities/v1 document with the setup inside it.`,
		...
		},
		{
			...
			Flags: append(append([]Flag{
				...
				{Name: "cache-env", Kind: FlagCustom, Doc: "`NAME=DIR` sets NAME to DIR inside the candidate checkout's .magus for every hook, so the cache it names is the candidate's own; repeatable"},
		...
		},
		{
			...
			Flags: append(append([]Flag{
				...
				{Name: "cache-env", Kind: FlagCustom, Doc: "`NAME=DIR` sets NAME to DIR inside the rebuild checkout's .magus for the regeneration, so the cache it names is that rebuild's own; repeatable"},
			}, queueFacts...), queueCheckout...),
		},
	},
	Examples: []Example{
