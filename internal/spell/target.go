...
// map keys in the session's synthetic/source module tables, and a bare "magus/spell"
...
var SpellModuleSource = strings.Join([]string{TargetModuleSource, PatchOpSource, CharmTypeSource, CommandSource, ServiceSource}, "\n")
...
// pure-types magus/spell module. Such a spell's op handlers
// run a declared command via the injected run-callback and carry no host side
// effects, so their specs can be extracted once at spec-resolution time (see
// resolveOps). A spell importing host modules (e.g. http) is a handler op spell
// whose handlers must be dispatched at invoke time instead.
