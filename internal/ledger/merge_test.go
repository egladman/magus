...
func applyMerge(t *testing.T, params map[string]any, base types.DelegationUnit) types.DelegationUnit {
	t.Helper()
...
func TestMergeTouchesOnlyNamedFields(t *testing.T) {
	...
	base := types.DelegationUnit{ID: "u1", Goal: "original goal", Tier: "standard"}
...
func TestMergeAnExplicitEmptyValueClears(t *testing.T) {
	...
	base := types.DelegationUnit{ID: "u1", Goal: "original goal"}
...
func TestMergeListAcceptsBothWireForms(t *testing.T) {
	...
	spaceForm := applyMerge(t, map[string]any{"owned_paths": "a/b c/d"}, types.DelegationUnit{})
	assert.Equal(t, []string{"a/b", "c/d"}, spaceForm.OwnedPaths)

	arrayForm := applyMerge(t, map[string]any{"owned_paths": []any{"a/b", "c/d"}}, types.DelegationUnit{})
