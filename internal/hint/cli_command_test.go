...
// declaredCommands returns every "magus ..." path declared with cmd() in cli_command.go,
...
func declaredCommands(t *testing.T) []string {
	t.Helper()
	f, err := parser.ParseFile(token.NewFileSet(), "cli_command.go", nil, 0)
	if err != nil {
		t.Fatalf("parsing cli_command.go: %v", err)
	}
	var out []string
