...
//go:generate go run ../../cmd/magus-bindings-gen -module os -lang lua -out gen/lua/os.go
//go:generate go run ../../cmd/magus-bindings-gen -module os -lang buzz -out gen/buzz/os.go
...
func runResult(ctx context.Context, name string, args []string, dir, label, cmd string, opts map[string]any) (map[string]any, error) {
	overrides, _ := ctx.Value(withEnvKey{}).([]string)
	res, err := run.Exec(ctx, name, args, run.ExecSpec{
		Dir:     dir,
		Env:     overrides,
	...
	if res.Code != 0 && !optBoolDefault(opts, "allow_failure", false) {
		if !res.Started {
			...
			return nil, fmt.Errorf("%s %s: %w", label, cmd, err)
		}
		return nil, fmt.Errorf("%s %s: exit %d", label, cmd, res.Code)
	}
...
func OsExec(ctx context.Context, cmd string, args []string, dir string, opts map[string]any) (map[string]any, error) {
...
func OsExecSh(ctx context.Context, line string, dir string, opts map[string]any) (map[string]any, error) {
