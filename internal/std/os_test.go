...
func TestOsExecResolvesCwd(t *testing.T) {
	...
	runPwd := func(ctxCwd, dirArg, wantDir string) {
		...
		require.NoError(t, err)
		require.Equal(t, 0, res["code"].(int), "pwd exit code")
...
func TestFsStat(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, int64(5), st["size"].(int64))
	assert.False(t, st["is_dir"].(bool), "is_dir should be false")
	assert.Equal(t, int64(0o640), st["mode"].(int64))
	_, ok := st["mtime"].(float64)
	assert.True(t, ok, "mtime is %T, want float64", st["mtime"])
