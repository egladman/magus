...
// list every module that has a gen/buzz trampoline; magusModules in the bindings
...
func TestBuzzBindingsMatchHostModules(t *testing.T) {
	...
	for _, m := range std.All() {
		...
		for _, meth := range m.Methods {
			...
			_, ok := mod.MapGet(key)
			assert.Truef(t, ok, "buzz %s.%s is missing (host declares it as %q); gen/buzz has drifted from std.Module",
