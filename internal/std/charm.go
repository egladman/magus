...
//go:generate go run ../../cmd/magus-bindings-gen -module charm -lang lua -out gen/lua/charm.go
//go:generate go run ../../cmd/magus-bindings-gen -module charm -lang buzz -out gen/buzz/charm.go
...
var Charm = Module{
	...
	Methods: []Method{
		...
		{
			Name:    "remove",
			Doc:     "Remove the first argv element equal to anchor.",
			Args:    []Arg{{Name: "argv", Type: TypeStringSlice}, {Name: "anchor", Type: TypeString}},
			Returns: []Ret{{Type: TypeAnyMap}}, Impl: CharmRemove,
		},
		{
			Name:    "after_func",
		...
		{
			Name:    "remove_func",
			Doc:     "Remove the first argv element for which fn(s) is truthy.",
			Args:    []Arg{{Name: "argv", Type: TypeStringSlice}, {Name: "fn", Type: TypeFunc}},
			Returns: []Ret{{Type: TypeAnyMap}}, Impl: CharmRemoveFunc,
		},
		{
			Name:    "path",
...
// CharmRemove implements charm.remove.
func CharmRemove(_ context.Context, argv []string, anchor string) (map[string]any, error) {
	i, err := anchorIndex(argv, anchor)
	if err != nil {
		return nil, err
	}
...
// CharmRemoveFunc implements charm.remove_func.
func CharmRemoveFunc(ctx context.Context, argv []string, fn Callback) (map[string]any, error) {
	i, err := anchorIndexFunc(ctx, argv, fn)
	if err != nil {
		return nil, err
	}
