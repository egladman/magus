...
func MagusCmd(ctx context.Context, args []string) (map[string]any, error) {
	self, err := os.Executable()
	if err != nil {
		return nil, fmt.Errorf("magus.cmd: executable: %w", err)
	}
	...
	if err := proc.RunChildSync(ctx, lim, runFn); err != nil {
		return nil, fmt.Errorf("magus.cmd: %w", err)
	}
	return rec, cmdErr
}
