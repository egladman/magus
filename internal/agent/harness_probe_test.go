...
// config that carries the declared "sh magus-guard-command.sh" entry byte-for-byte
...
func TestVerifyHarnessProbeCatchesABrokenWiredCommand(t *testing.T) {
	...
	writeProbeableHarness(t, root, "broken", "sh magus-guard-command.sh")
	...
	assert.Contains(t, result.Reason, "magus-guard-command.sh")
}
...
func TestVerifyHarnessProbeAcceptsAWorkingWiredCommand(t *testing.T) {
	...
	writeProbeableHarness(t, root, "working", "sh magus-guard-command.sh")
...
func TestVerifyHarnessProbeAnswersWithAWrongDecisionIsUncovered(t *testing.T) {
	root := t.TempDir()
	writeFakeMagusBinary(t, root)
	writeStubGuardScript(t, root, "magus-guard-command.sh", "pass")
	writeProbeableHarness(t, root, "wrongdecision", "sh magus-guard-command.sh")
...
func TestVerifyHarnessProbeReportsMissingJQAsUnprobed(t *testing.T) {
	...
	writeFakeMagusBinary(t, root)
	writeStubGuardScript(t, root, "magus-guard-command.sh", "deny")
	writeProbeableHarness(t, root, "nojq", "sh magus-guard-command.sh")
...
func TestVerifyHarnessProbeReportsNoMagusBinaryAsUnprobed(t *testing.T) {
	...
	writeStubGuardScript(t, root, "magus-guard-command.sh", "deny")
	writeProbeableHarness(t, root, "nomagus", "sh magus-guard-command.sh")
