...
// 51: magus-handoff-journal teaches what `put` writes. It sets the fields the
// caller sends and keeps the rest, so refreshing a status no longer drops the
// body beside it; clearing a field is a delete and a create; a record cannot be
// updated into another type; and `allow_missing: false` (CLI `--amend`) says the
// entry is meant to exist.
...
var skillSources = []skillSource{
	...
	{name: "magus-handoff-journal", description: "Maintain a user-owned handoff journal through magus_memory or `magus memory`: named decisions, plans, pointers, and the hypotheses an investigation ruled out, all surviving worktrees and sessions. Use when a debugging session eliminates a possibility a later session would otherwise re-propose. It is not automatic agent memory; add an entry only when a later person needs to reopen the linked graph/query/output/doc evidence. Verify malformed, stale, broken-linked, and unresolvable-evidence entries before relying on them.", bodyPath: "skills/magus-handoff-journal/SKILL.md", formerNames: []string{"magus-memory"}},
