...
// and returns a store resolved exactly as checkLeaseBinding resolves its own
...
func TestLeaseBindingAdvisesOnATerminalBoundRow(t *testing.T) {
...
func TestLeaseBindingAdvisesOnALiveRowWithNoRegisteredBase(t *testing.T) {
	cacheDir, root, store := tmpLedger(t)
...
func TestLeaseBindingPassesOnALiveRegisteredRow(t *testing.T) {
	cacheDir, root, store := tmpLedger(t)
