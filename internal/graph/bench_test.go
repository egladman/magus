...
func linear(b *testing.B, n int) *graph.Graph {
	b.Helper()
	bl := graph.New()
	ids := make([]graph.ID, n)
	for i := range n {
...
func binTree(b *testing.B, nodes int) *graph.Graph {
	b.Helper()
	bl := graph.New()
	ids := make([]graph.ID, nodes)
	for i := range nodes {
...
func diamond(b *testing.B, n int) *graph.Graph {
	b.Helper()
	bl := graph.New()
	addN := func(s string) graph.ID { return bl.AddNode(s) }
	prev := addN("d0000-bot")
	for i := 0; i < n/4; i++ {
...
func layered(b *testing.B, layers, width int) *graph.Graph {
	b.Helper()
	bl := graph.New()
	grid := make([][]graph.ID, layers)
	for l := range layers {
		grid[l] = make([]graph.ID, width)
		for w := range width {
...
func BenchmarkReverseClosure_seed1_linear(b *testing.B) {
	for _, n := range []int{100, 1_000, 10_000} {
		b.Run(fmt.Sprintf("n=%d", n), func(b *testing.B) {
			...
			dst := make([]graph.ID, 0, n)
			b.ResetTimer()
			for b.Loop() {
				dst = g.ReverseClosure(dst[:0], []graph.ID{leaf})
			}
		})
	}
}
...
func BenchmarkPathsFromSeeds_linear(b *testing.B) {
	for _, n := range []int{100, 1_000, 10_000} {
		b.Run(fmt.Sprintf("n=%d", n), func(b *testing.B) {
			...
			out := make([]graph.AffectedPath, 0, 4)
			b.ResetTimer()
			for b.Loop() {
				out = g.PathsFromSeeds(target, []graph.ID{seed}, out)
			}
		})
	}
}
