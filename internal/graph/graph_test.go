...
func build(t *testing.T, entries [][]string) *graph.Graph {
	t.Helper()
	b := graph.New()
	bids := map[string]graph.ID{}
...
func pid(t *testing.T, g *graph.Graph, path string) graph.ID {
	t.Helper()
	id, ok := g.ID(path)
	if !ok {
...
func pathOf(t *testing.T, g *graph.Graph, id graph.ID) string {
	t.Helper()
	p, ok := g.Path(id)
	if !ok {
...
func chainPaths(g *graph.Graph, ids []graph.ID) []string {
...
func TestObserverOnErrorFiresOnCycle(t *testing.T) {
	...
	_ = b.AddEdge(a, c)
	_ = b.AddEdge(c, a)
	_, err := b.Build(graph.WithObserver(obs))
	if err == nil {
		t.Fatal("Build: expected ErrCycle, got nil")
	}
	if len(seen) != 1 || !errors.Is(seen[0], graph.ErrCycle) {
...
func TestPathOutOfRange(t *testing.T) {
	t.Parallel()
	g := build(t, [][]string{{"a"}})
	if p, ok := g.Path(graph.NoID); ok || p != "" {
		t.Errorf("Path(NoID): got (%q,%v), want (\"\",false)", p, ok)
	}
	if p, ok := g.Path(graph.ID(999)); ok || p != "" {
...
func TestReverseClosureLinear(t *testing.T) {
	...
	g := build(t, [][]string{{"a", "b"}, {"b", "c"}, {"c"}})
	c := pid(t, g, "c")
	got := g.ReverseClosure(nil, []graph.ID{c})
	if len(got) != 3 {
...
func TestReverseClosureIncludesSeeds(t *testing.T) {
	...
	id := pid(t, g, "solo")
	got := g.ReverseClosure(nil, []graph.ID{id})
...
func TestReverseClosureRejectsBadSeed(t *testing.T) {
	...
	got := g.ReverseClosure(nil, []graph.ID{graph.ID(999), a, graph.NoID})
...
func TestNCCDEmptyEmitsEvent(t *testing.T) {
	t.Parallel()
	var seen []graph.QueryEvent
	obs := &captureObserver{onQuery: func(e graph.QueryEvent) { seen = append(seen, e) }}

	b := graph.New()
	g, err := b.Build(graph.WithObserver(obs))
	if err != nil {
		t.Fatalf("Build: %v", err)
	}
	_ = g.NCCD()
...
func TestPathsFromSeedsDirect(t *testing.T) {
	...
	id := pid(t, g, "api")
	paths := g.PathsFromSeeds(id, []graph.ID{id}, nil)
...
func TestPathsFromSeedsTransitive(t *testing.T) {
	...
	a := pid(t, g, "a")
	c := pid(t, g, "c")
	paths := g.PathsFromSeeds(a, []graph.ID{c}, nil)
	if len(paths) != 1 {
...
func TestPathsFromSeedsUnreachable(t *testing.T) {
	...
	api := pid(t, g, "api")
	web := pid(t, g, "web")
	paths := g.PathsFromSeeds(api, []graph.ID{web}, nil)
	if len(paths) != 0 {
...
func TestPathsFromSeedsMultiple(t *testing.T) {
	...
	sa := pid(t, g, "seed-a")
	sb := pid(t, g, "seed-b")
	paths := g.PathsFromSeeds(target, []graph.ID{sa, sb}, nil)
	if len(paths) != 2 {
...
func TestPathsFromSeedsRejectsBadTarget(t *testing.T) {
	...
	g := build(t, [][]string{{"a"}})
	a := pid(t, g, "a")
	if got := g.PathsFromSeeds(graph.NoID, []graph.ID{a}, nil); len(got) != 0 {
		t.Errorf("NoID target: got %v, want empty", got)
	}
	if got := g.PathsFromSeeds(graph.ID(999), []graph.ID{a}, nil); len(got) != 0 {
...
func TestFanOutDoesNotMutateCallerSlice(t *testing.T) {
	t.Parallel()
	a := graph.NoopObserver{}
	b := graph.NoopObserver{}
	caller := []graph.Observer{a, nil, b}
	_ = graph.FanOut(caller...)
...
type captureObserver struct {
	onBuild func(graph.BuildStats)
	onQuery func(graph.QueryEvent)
	onErr   func(error)
}

func (c *captureObserver) OnBuild(s graph.BuildStats) {
	if c.onBuild != nil {
		c.onBuild(s)
	}
}

func (c *captureObserver) OnQuery(e graph.QueryEvent) {
	if c.onQuery != nil {
		c.onQuery(e)
	}
}
