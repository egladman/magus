...
//
// On top of the live directive, the caller can pre-apply the same fragment
// directives the page already honors on load (console/src/console/graph/main.ts
// applyDeepLinks): `q=` auto-runs a search, and `view=` activates a named view
// with optional `node=`/`to=` foci. Only directives that are set are emitted.
//
// The helper is pure - it FORMATS a URL from values the caller has already
// discovered (the loopback host via mcpAddrString(), the token via auth.Load()).
// It does not probe the daemon or read config; that discovery stays with the
...
// ErrNoDaemon is returned when there is nothing to link to - the caller supplied
// an empty Host or Token, which is how it signals "no reachable daemon". Callers
// treat this as "omit the link", not as a hard failure.
var ErrNoDaemon = errors.New("graphurl: no reachable daemon; omit the link")

// GraphLinkOpts is the input to GraphLink. Host and Token come from the daemon
// discovery the caller already performs (mcpAddrString() and auth.Load(), the
// graphOpenLive mechanism); the four directive fields are all optional.
type GraphLinkOpts struct {
	...
	// before any request). Empty Host means "no daemon" -> ErrNoDaemon.
	Host string
	...
	// Empty Token means "no daemon" -> ErrNoDaemon.
	Token string
...
// It returns ErrNoDaemon when Host or Token is empty (nothing to link to). The
// resulting fragment is `#live=<host>&token=<tok>[&q=][&view=][&node=][&to=]`,
// with only the directives that are set included, and every value percent-encoded
// so the page's decodeURIComponent-based hash parser reads it back exactly.
func GraphLink(opts GraphLinkOpts) (string, error) {
	...
	// (the page parses it as a URL host), then the token. Then the optional
	// view/query directives, in a fixed order for stable output.
	parts := []string{
		"live=" + opts.Host,
		"token=" + encodeComponent(opts.Token),
	}
	if opts.Query != "" {
