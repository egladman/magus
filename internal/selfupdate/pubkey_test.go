...
func TestReleaseTrustAnchorMatchesInstallerAndCI(t *testing.T) {
	...
	require.NoError(t, err)
	// The key lives on the verify page, not the setup hub: a first install must
	...
	assert.Contains(t, string(guide), "/7uPpvNidN79EoiAk8ajIsJTK8VFAW9JWrSVXey2Z3k=", "setup guide must publish the binary's release key")
}
