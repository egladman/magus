...
// procedureNeeds is the Need of every Connect procedure the daemon serves, keyed by service
// then method. serviceNeeds refuses a service whose methods and entries here disagree, so a
// procedure added to a proto without a Need stops the daemon from starting.
...
func apiNeedsOf(path string) (map[string]types.Need, error) {
	...
	if !ok {
		return nil, fmt.Errorf("daemon: route %s has no Need", path)
	}
...
func serviceNeeds(path string) (map[string]types.Need, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("daemon: service %s: %w", name, err)
	}
	sd, ok := d.(protoreflect.ServiceDescriptor)
	if !ok {
		return nil, fmt.Errorf("daemon: %s is not a service", name)
	}
	table, ok := procedureNeeds[name]
	if !ok {
		return nil, fmt.Errorf("daemon: service %s has no Needs", name)
	}
	methods := sd.Methods()
	if methods.Len() != len(table) {
		return nil, fmt.Errorf("daemon: service %s has %d methods and %d Needs", name, methods.Len(), len(table))
	}
	...
	for i := range methods.Len() {
		...
		need, ok := table[m]
		if !ok {
			return nil, fmt.Errorf("daemon: procedure %s/%s has no Need", name, m)
		}
