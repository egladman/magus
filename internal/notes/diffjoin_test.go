...
func TestDriftStatusIsCarriedThrough(t *testing.T) {
	...
	got := AnchorsTouching([]ResolvedAnchor{drifted, dangling, clean},
		[]string{cacheFile}, []string{cachePut})
...
func TestAnchorsTheDiffDoesNotTouchAreAbsent(t *testing.T) {
	got := AnchorsTouching([]ResolvedAnchor{
...
func TestOneAnchorCollapsesToItsStrongestMatch(t *testing.T) {
	...
	require.Len(t, got, 1, "the same-file match must not be reported alongside the exact one")
	assert.Equal(t, MatchSymbol, got[0].Match)
}
...
func TestOrderIsByNoteThenPositionWhateverTheInputOrder(t *testing.T) {
	...
	want := AnchorsTouching(in, []string{cacheFile}, []string{cachePut})
	...
	for i := range in {
		...
		assert.Equal(t, want, AnchorsTouching(shuffled, []string{cacheFile}, []string{cachePut}),
			"rotation by %d", i)
	}
	reversed := slices.Clone(in)
	slices.Reverse(reversed)
	assert.Equal(t, want, AnchorsTouching(reversed, []string{cacheFile}, []string{cachePut}))
}

func TestEmptyInputsMatchNothing(t *testing.T) {
	assert.Empty(t, AnchorsTouching(nil, []string{cacheFile}, []string{cachePut}))
	assert.Empty(t, AnchorsTouching(
