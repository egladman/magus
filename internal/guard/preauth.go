...
func servedNextPreauthorizes(gate hint.Gate, command string) string {
	if gate.CacheDir() == "" {
		return ""
	}
...
func readServedNext(gate hint.Gate) []hint.ServedNextEntry {
	entries := readServedNextFile(hint.ServedNextPath(gate.CacheDir()))
...
func readServedNextFile(journal string) []hint.ServedNextEntry {
	body, err := os.ReadFile(journal)
	if err != nil {
		return nil
	}
	lines := strings.Split(strings.TrimRight(string(body), "\n"), "\n")
	entries := make([]hint.ServedNextEntry, 0, len(lines))
...
// anything else.
//
// Deliberately NOT ParseCommands: that one peels sudo/env/sh -c to find the
