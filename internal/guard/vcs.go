...
func gitGuard(cmds []hint.Invocation) (ShellVerdict, bool) {
	...
	for _, c := range cmds {
		...
		switch sub {
		case "push":
			return ShellVerdict{Context: pushGuardContext}, true
		case "add":
		...
		case "checkout":
			...
			if slices.Contains(rest, "--") {
				return ShellVerdict{Context: revertGuardContext}, true
			}
		case "restore":
		...
		case "describe":
			...
			return ShellVerdict{Context: checkpointGuardContext}, true
		case "stash":
