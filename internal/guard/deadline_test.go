...
func TestGradeLeasedWriteDeniesAnOverdueLease(t *testing.T) {
	...
	require.Equal(t, "deny", got.Decision, "an overdue lease writing inside its own lane is denied")
...
func TestGradeLeasedWriteIgnoresAnOverdueOwner(t *testing.T) {
	...
	assert.NotContains(t, got.Reason, "is owned by lease lease-a", "an overdue lease no longer holds its lane against others")
}
