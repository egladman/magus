...
// BenchmarkExtract measures the static parse of the real magusfile.bzz — the
...
func BenchmarkExtract(b *testing.B) {
	src, err := os.ReadFile(filepath.Join("..", "..", "magusfile.bzz"))
	if err != nil {
		b.Fatalf("read magusfile.bzz: %v", err)
	}
	s := string(src)
	b.ReportAllocs()
	b.ResetTimer()
