...
// into a [ispell.Descriptor], so a Buzz spell and a Teal spell read identically.
...
func Extract(ctx context.Context, src string) (ispell.Descriptor, error) {
	...
	if err := sess.Exec(ctx, src); err != nil {
		return ispell.Descriptor{}, fmt.Errorf("buzzspell: exec: %w", err)
	}
