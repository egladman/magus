...
// by the engine's debug API. Source is the chunk name (e.g. "@magusfile.tl"
// for Lua, "<magusfile>" for JS); ShortSrc is a truncated form suitable for
// display; CurrentLine is the 1-based line number, or -1 if unknown.
type Frame struct {
	Source      string
	ShortSrc    string
	CurrentLine int
	Name        string // function name when discoverable
	What        string // "Lua"/"main"/"G"/"tail" for Lua
}
...
//
// Each engine exposes one driver per language it speaks (Lua engines have
// "lua" and "teal"). The REPL switches between drivers when the user types
// .tl / .lua.
type ReplDriver interface {
	// Language returns the driver's name ("lua", "teal").
	Language() string
	...
	// printable value. Multiple return values are supported (Lua).
	EvalLine(snippet string) ([]Value, error)

	// IsIncomplete returns true when err indicates that snippet is a partial
	// statement that needs more input (e.g. Lua's "<eof>" message). The REPL
	// accumulates additional lines when this returns true.
