...
func registerCryptoBytes() buzz.Value {
	m := buzz.NewMap()
	...
	m.MapSet("hmacSha256", buzz.DirectValue("crypto.hmacSha256", func(_ context.Context, args []buzz.Value) (buzz.Value, error) {
	...
	m.MapSet("hmacSha256Hex", buzz.DirectValue("crypto.hmacSha256Hex", func(_ context.Context, args []buzz.Value) (buzz.Value, error) {
		sum, err := cryptoHMACSum(args)
		if err != nil {
			return buzz.Null, err
		}
	...
	m.MapSet("base64Encode", buzz.DirectValue("crypto.base64Encode", func(_ context.Context, args []buzz.Value) (buzz.Value, error) {
		b, err := cryptoBytesArg(args, 0)
		if err != nil {
			return buzz.Null, fmt.Errorf("crypto.base64Encode: %w", err)
		}
	...
	m.MapSet("base64Decode", buzz.DirectValue("crypto.base64Decode", func(_ context.Context, args []buzz.Value) (buzz.Value, error) {
		if len(args) < 1 || !args[0].IsStr() {
			return buzz.Null, fmt.Errorf("crypto.base64Decode: requires a str argument")
		}
		b, err := base64.StdEncoding.DecodeString(args[0].AsString())
		if err != nil {
			return buzz.Null, fmt.Errorf("crypto.base64Decode: %w", err)
		}
...
// a [int] byte list.
func cryptoHMACSum(args []buzz.Value) ([]byte, error) {
...
func cryptoBytesArg(args []buzz.Value, i int) ([]byte, error) {
	if i >= len(args) {
...
func cryptoBytesValue(b []byte) buzz.Value {
