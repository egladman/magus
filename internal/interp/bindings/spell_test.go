...
func TestDispatchOpInjectsDeclaredSecrets(t *testing.T) {
	...
	_, err := dispatchOp(ctx, ops, spells.InvokeRequest{Target: "publish", Dir: dir})
	require.NoError(t, err)
...
func TestDispatchOpSecretsRequireAResolver(t *testing.T) {
	...
	_, err := dispatchOp(context.Background(), ops, spells.InvokeRequest{Target: "publish", Dir: t.TempDir()})
	require.Error(t, err)
...
func TestDispatchOpSecretsPropagateResolverError(t *testing.T) {
	...
	_, err := dispatchOp(ctx, ops, spells.InvokeRequest{Target: "publish", Dir: t.TempDir()})
	require.Error(t, err)
	assert.Contains(t, err.Error(), `"publish"`)
