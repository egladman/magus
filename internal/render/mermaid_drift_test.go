...
// JS implementation in website/js/graph-explorer.js:
//
//  1. It runs the Go emitters and asserts the expected classDef names appear in
//     their output.
//  2. It reads website/js/graph-explorer.js from disk and asserts the SAME
...
// website/js/editor.js for the house pattern this follows.
//
...
func TestMermaidClassDefDrift(t *testing.T) {
	...
	// -- side B: verify website/js/graph-explorer.js contains the same names ----

	_, thisFile, _, ok := runtime.Caller(0)
	require.True(t, ok, "runtime.Caller failed")
	repoRoot := filepath.Join(filepath.Dir(thisFile), "..", "..")
	jsPath := filepath.Join(repoRoot, "website", "js", "graph-explorer.js")
	...
	t.Run("js_targets_classDefs", func(t *testing.T) {
		...
		for _, name := range targetsClassDefNames {
			require.True(t, strings.Contains(content, "classDef "+name+" "),
				"JS file missing targets classDef %q - update website/js/graph-explorer.js "+
	...
	t.Run("js_knowledge_classDefs", func(t *testing.T) {
		...
		for _, name := range knowledgeClassDefNames {
			require.True(t, strings.Contains(content, name+":"),
				"JS file missing knowledge classDef %q - update website/js/graph-explorer.js "+
