...
func TestWriteTargetGraphMarkdown(t *testing.T) {
	...
	// With a routing carrying one, the provenance line names the build that rendered the
	// file. That is the point: regenerating with a magus from another branch changes this
	// line, so the cause shows up in the diff instead of reading as ordinary drift.
	t.Run("catalog fingerprint appears when routing carries one", func(t *testing.T) {
		var fb bytes.Buffer
		...
		assert.Contains(t, fb.String(),
			"> Generated by `magus run generate:rw .` (catalog `deadbeefcafe0123`). Do not edit by hand.")
	})
