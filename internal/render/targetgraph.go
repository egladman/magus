...
func WriteTargetGraphMarkdown(w io.Writer, out types.TargetGraphOutput, routing *types.KnowledgeRouting, explorerURL string, defaultCharms []string) error {
	...
	// query tools - not this file - are the source of truth. The catalog fingerprint
	// names the build that wrote it, so a diff caused by regenerating with a magus from
	// another branch is legible as that rather than as an ordinary change.
	provenance := "Generated by " + md.Code("magus run generate:rw .") + ". Do not edit by hand."
	if routing != nil && routing.CatalogFingerprint != "" {
		provenance = "Generated by " + md.Code("magus run generate:rw .") +
			" (catalog " + md.Code(routing.CatalogFingerprint) + "). Do not edit by hand."
	}
	b.Quote(provenance, "If the counts look stale, trust "+md.Code("magus query")+" over this file.")
