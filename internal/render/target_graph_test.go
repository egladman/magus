...
func TestWriteTargetGraphMarkdown(t *testing.T) {
	...
	// has one. It identifies the BINARY, not the content, and this file is drift-gated:
	// two builds of the same source produced different fingerprints, so a regeneration
	// that changed nothing still rewrote the line and failed CI with the fingerprint as
	// the entire diff. Attribution lives in the JSON export instead.
	...
	for _, want := range []string{
		...
		// The compact per-project list is a table, padded the way dprint writes one.
		"| Target  | What it does      |",
	...
	assert.Less(t, strings.Index(got, "## Route by question"), strings.Index(got, "| Target"),
