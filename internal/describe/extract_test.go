...
// TestExecTools pins the direct-exec attribution: a target that shells out instead of
// composing a spell op is not opaque to the static read. Only a string-literal command
// is attributed, and it is reduced to a basename so a vendored path and a bare name are
// one tool rather than two.
func TestExecTools(t *testing.T) {
	src := `import "magus";
import "os";

export fun bespoke(ctx: magus\Context, args: [str]) > void {
    os\exec("pnpm", ["run", "build"], ".", {});
    os\exec("./node_modules/.bin/tsc", ["-b"], ".", {});
    os\exec("pnpm", ["run", "test"], ".", {});
}

export fun dotted(ctx: magus\Context, args: [str]) > void { os.exec("git", ["status"], ".", {}); }
...
export fun viaHelper(ctx: magus\Context, args: [str]) > void { runIt(); }

fun runIt() > void { os\exec("shellcheck", ["x.sh"], ".", {}); }
`
	g := Extract(src)

	n, ok := nodeByName(g, "bespoke")
	require.True(t, ok)
	assert.Equal(t, []string{"pnpm", "tsc"}, n.Tools,
		"deduped, first-appearance order, basename only")

	n, ok = nodeByName(g, "dotted")
	require.True(t, ok)
	assert.Equal(t, []string{"git"}, n.Tools, "the older os.exec spelling is the same call")

	n, ok = nodeByName(g, "computed")
	require.True(t, ok)
	assert.Empty(t, n.Tools, "a computed command is left out, not guessed at")

	n, ok = nodeByName(g, "via-helper") // node names are kebab-normalized
	require.True(t, ok)
	assert.Equal(t, []string{"shellcheck"}, n.Tools,
		"a target factoring its exec into a same-file helper keeps the attribution")
}
