...
func TestFetchBranchLeavesTagsAndTrackingRefsAlone(t *testing.T) {
	f := newRemoteFixture(t)
	before := refsOf(t, f.clone)
	tip := f.advance(t, "main", "next.txt")

	got, err := gitVCS{}.FetchBranch(t.Context(), f.clone, "origin", "main")
...
func TestFetchBranchConcurrentCallsDoNotShareARef(t *testing.T) {
	f := newRemoteFixture(t)
	...
	for i := range 16 {
		...
		wg.Go(func() {
			id, err := gitVCS{}.FetchBranch(t.Context(), f.clone, remote, "main")
			if err != nil {
				errs <- err
				return
			}
...
func TestRemoteArgumentsRefuseRefspecsOptionsAndURLs(t *testing.T) {
	...
	require.Error(t, err)
	_, err = g.FetchBranch(ctx, f.clone, "origin", "main:main")
	require.Error(t, err)
	_, err = g.FetchBranch(ctx, f.clone, "file://"+f.remote, "main")
	require.Error(t, err, "a URL is not a configured remote's name")
	_, err = g.FetchBranch(ctx, f.clone, "--upload-pack=touch x", "main")
	require.Error(t, err)
	require.Error(t, g.FetchCommit(ctx, f.clone, "origin", "HEAD"), "a revision expression is not a commit id")
	require.Error(t, g.Push(ctx, f.clone, "origin", "refs/heads/*", id, id))
...
// B7: a lease that no longer holds is ErrPushLease, and a refusal of the remote's own is a
...
func TestPushLeaseAndRejection(t *testing.T) {
	...
	require.NoError(t, g.Push(ctx, f.clone, "origin", "refs/heads/main", update, base))
	...
	err := g.Push(ctx, f.clone, "origin", "refs/heads/main", late, base)
	require.ErrorIs(t, err, types.ErrPushLease, "the branch moved since base")

	err = g.Push(ctx, f.clone, "origin", "refs/heads/gone", update, base)
	require.ErrorIs(t, err, types.ErrPushLease, "a lease never creates a branch")
	...
	err = g.Push(ctx, f.clone, "origin", "refs/heads/main", refused, update)
	...
	assert.NotErrorIs(t, err, types.ErrPushLease, "a refusal retrying cannot fix is not a lease failure")
}
...
func TestCommitPushAndCheckoutRunNoHooks(t *testing.T) {
	...
	id, err := g.Commit(ctx, f.clone, types.CommitOptions{
		Message: "add new", Author: queueIdentity, Committer: queueIdentity, Paths: []string{"new.txt"},
	})
	require.NoError(t, err)
	require.NoError(t, g.Push(ctx, f.clone, "origin", "refs/heads/main", id, base))
...
func TestCommitRecordsLiteralPaths(t *testing.T) {
	...
	id, err := gitVCS{}.Commit(t.Context(), dir, types.CommitOptions{
...
func TestCommitRefusesAnIncompleteIdentityAndAnEmptyCommit(t *testing.T) {
	...
	_, err := gitVCS{}.Commit(t.Context(), dir, types.CommitOptions{Message: "x", Author: queueIdentity})
	require.ErrorContains(t, err, "committer")
	_, err = gitVCS{}.Commit(t.Context(), dir, types.CommitOptions{Message: "x", Author: queueIdentity, Committer: queueIdentity})
...
func TestCommitTreeIsDeterministicAndCreatesNoRef(t *testing.T) {
	...
	before := refsOf(t, dir)
	c := types.TreeCommit{
		Tree: tree, Parents: []string{revs["ours"], revs["clean"]}, Message: "update",
		Author: types.Person{Name: "author", Email: "a@x"}, Committer: queueIdentity,
...
func TestRangeFilesReportsBothSidesOfARename(t *testing.T) {
	...
	got, err := gitVCS{}.RangeFiles(t.Context(), dir, base, "HEAD")
	require.NoError(t, err)
...
func TestBundleCarriesCommitsWithoutRefs(t *testing.T) {
	...
	require.NoError(t, g.Bundle(ctx, f.clone, file, base, carried))
...
func TestGitCallsIgnoreAHostileGitDir(t *testing.T) {
	...
	t.Run("hostile environment", func(t *testing.T) {
		...
		_, err = gitVCS{}.Commit(t.Context(), clone, types.CommitOptions{Message: "x", Author: queueIdentity, Committer: queueIdentity})
...
func TestGitExecPinsTheSettingsItsParsersDependOn(t *testing.T) {
	...
	cmd := gitExec(t.Context(), "/repo", gitOpts{NoHooks: true, Literal: true}, "status")

	joined := strings.Join(cmd.Args, " ")
	for _, pin := range []string{"color.ui=false", "core.quotePath=false", "log.showSignature=false",
		"rerere.enabled=false", "commit.gpgSign=false", "core.hooksPath=" + os.DevNull} {
