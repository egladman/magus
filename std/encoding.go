...
//go:generate go run ../cmd/magus-bindings-gen -module encoding -lang buzz -out ../hostbuzz/gen/encoding.go
...
func EncodingParseURL(_ context.Context, rawURL string) (types.URL, error) {
	u, err := url.Parse(rawURL)
	if err != nil {
		return types.URL{}, fmt.Errorf("encoding.parse_url: %w", err)
	}
	return types.URL{
		Scheme:   u.Scheme,
		Host:     u.Hostname(),
