---
title: Delegation
description: The surface magus gives an agent that fans work out - the working-state checkpoint, the declared delegation ledger, the console Plan surface, the recorded spawn, and verifying a unit against the diff since its checkpoint.
tags:
  [
    agents,
    delegation,
...
partition by write set rather than by affected project, prove the units cannot
collide, bound the fan-out, match a model to each unit. This page is the surface
that skill writes to and reads from.

| step                     | surface                                        |
| ------------------------ | ---------------------------------------------- |
| Record the working state | `magus vcs checkpoint`, `magus_vcs_checkpoint` |
| Hand out the units       | the host's own spawn - recorded, never judged  |
| Declare the plan         | `magus_ledger` (`op=put`)                      |
| Watch it                 | the console Plan surface, `GET /api/v1/ledger` |
| Verify                   | the actual diff since each unit's checkpoint   |
...
nothing changed anywhere - so taking one per delegated unit costs the tree
...
unit's checkpoint, which is the last step below.

**Every row ends in `pass`, `fail`, or `no_return`.** `no_return` is not a
failure. A unit that failed came back and said so; a unit that died, stalled, or
...
nothing. A read-only unit that gathers evidence and writes nothing carries an
...
magus unit and magus will not infer one from prose, so the unit is stamped only
...
join writes the marker; one that does not gets an event with no unit, which is a
missing join rather than a wrong one. A `unit:` line quoted deeper in a prompt
...
1. Diff the actual tree against the unit's checkpoint, and compare THAT against
...
- Block a write outside a unit's owned paths, or any write at all on account of
  a ledger row.
