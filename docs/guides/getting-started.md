...
Bind the built-in `go` spell by importing it and listing it in `magus\project`, then compose its ops into your targets. Op keys are the CLI command in kebab-case, so they are reached by subscript (`golang["go-build"]`), and every op call takes the target's `ctx` as its first argument:
...
Charms are shared, composable execution modifiers attached after `:`; see [charms.md](../concepts/charms.md). If you want autofix as the local default so you do not type `:rw` each time, set `default_charms: [rw]` in `magus.yaml` (charms.md covers the safeguards that keep CI read-only regardless).
...
`magus affected ci` is the command your CI runs on every pull request. It keys off the `ci` anchor, computes the affected set from the VCS diff, and does the minimum work. For CI fan-out across runners, `magus affected --plan` emits a provider-neutral shard plan; see [`magus affected`](../reference/manpage/magus-affected.md) for `--plan`, `--stdin`, and bisect.
