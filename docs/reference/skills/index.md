...
| [magus-change-summary](magus-change-summary.md) | 7051 | 5385 | 23% | Summarize what changed in a magus workspace, write it up, or answer a granular diff question. |
| [magus-commit-composition](magus-commit-composition.md) | 4432 | 3824 | 13% | Restructure an UNPUSHED branch so each commit is one reviewable idea, using the workspace's own boundaries (project ownership, declared outputs, blast radius) rather than guessing from paths. |
| [magus-context-audit](magus-context-audit.md) | 5832 | 4173 | 28% | Audit the instructions an agent was given - the repo instruction file, installed skills, memory entries, a routing index, hook-injected text, and any user-level instruction file - for statements that contradict each other or that no longer match what the tools do. |
...
| [magus-memory](magus-memory.md) | 5557 | 4609 | 17% | Maintain a user-owned per-repository memory through magus_memory or `magus memory`: named decisions, plans, pointers, and the hypotheses an investigation ruled out, all surviving worktrees and sessions. |
| [magus-multi-agent](magus-multi-agent.md) | 21839 | 16102 | 26% | Split work across agents in a magus workspace as an acceptance-criteria loop: partition by WRITE SET using graph evidence (magus refs --occurrences, explain, affected --plan --stdin), prove the leases cannot collide, bound fan-out depth, and match each lease's model to the work it needs. |
| [magus-query](magus-query.md) | 13065 | 10541 | 19% | Query the magus knowledge graph to find and relate entities (projects, targets, spells, ops, charms, modules, diagnostics, docs). |
...
| [magus-workspace-rules](magus-workspace-rules.md) | 5437 | 4393 | 19% | Adapt magus's installed agent surface to THIS workspace without breaking it. |
| **all 14** | **134264** | **105302** | **21%** | |
