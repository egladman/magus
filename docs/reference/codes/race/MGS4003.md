...
This is an error, not a warning: it fails the run. Determinism is not a preference
here, because three things magus does are unsound without it - drift gating compares
regenerated output against what is committed, a cache hit replays stored bytes instead
of rerunning, and a conflict in generated output is resolved by regenerating. A
generator that emits different bytes for the same inputs breaks all three. The fix is
always in the generator.

It is only emitted by `--race=replay`, which:
...
This check is expensive: it doubles the wall-clock time of the affected build, and the
second pass cannot benefit from the cache. That is why it is opt-in and belongs on a
scheduled audit rather than on every push. Asking for it and getting a warning would be
the worst of both, so when you ask, you get a verdict.
