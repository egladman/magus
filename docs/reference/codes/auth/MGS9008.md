...
description: The daemon refused a request to a local-only route from a peer that is not on this machine's loopback interface.
tags: [MGS9008, auth, loopback, share, "403"]
---

# MGS9008: local access only

The daemon answered `403 forbidden` because the route only serves a caller on
...
daemon exposes to the network. Binding to loopback is the first defense; this
...
Make the request from the machine the daemon runs on, directly to
