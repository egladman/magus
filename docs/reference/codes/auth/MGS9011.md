...
description: The daemon refused a request to a guarded route because it carried no bearer token at all. The client was not configured with one.
...
token: the daemon reads the credential from the header only, so it never lands
in an access log.
