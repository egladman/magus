...
description: A share to another device was started while the daemon found no built console to serve it, answered 400.
tags: [MGS9013, auth, console, share, "400"]
---

# MGS9013: console not built

The daemon answered `400 bad request` to a share request because it found no
...
The daemon looks for the build in `MAGUS_CONSOLE_DIR`, then in
...
share again. The daemon reads the directory when it starts, so restart it
