---
title: "MGS9001: bearer token rejected"
description: The daemon refused a request because the bearer token it carried was wrong, expired, or revoked. This is the usual reason an MCP client cannot connect.
tags: [MGS9001, auth, mcp, connector, token, onboarding, bearer]
---

# MGS9001: bearer token rejected

The daemon received a request on a guarded route (`/mcp`, the console data
services) carrying a bearer token it would not accept, so it answered `401
unauthorized` with `WWW-Authenticate: Bearer realm="magus", error="invalid_token"`.
This is the most common reason an MCP client (Claude Code, an IDE, Desktop)
fails to connect.

```text
[MGS9001] the daemon rejected the bearer token: it is wrong, expired, or
revoked. Mint one with: magus config mcp connector create, or magus config
console token create
  see: .../MGS9001.md
```

## Why

Every guarded route requires a bearer token. The daemon does NOT say why it
refused one (that would let a caller probe which tokens exist), so any of these
produces the same `401`:

- **Wrong token** (a typo, or a token from a different daemon).
- **Expired** token (every stored token expires, at most 366 days out).
- **Revoked** token (revoked from the CLI or the console Settings).
- **Wrong class for this listener**: a share link's `mgl_` token on the
  loopback daemon, or any other token on a share link's listener.
- **Minted by an older magus**: a token from before the class prefix. Mint a new
  one ([MGS9016](MGS9016.md), [MGS9017](MGS9017.md) name the files).

A request that carried no token at all is [MGS9011](MGS9011.md) instead, and a
valid token whose grant is below the route's need is [MGS9015](MGS9015.md).

## Resolution

1. Mint a connector token and read how to wire it into your client:

   ```sh
   magus config mcp connector create --name my-client
   ```

2. Confirm the token your client sends matches one the daemon knows:

   ```sh
   magus config mcp connector ls
   ```

   A token you expect but do not see was revoked or belongs to another daemon.

3. If it is present but still rejected, it may be expired: mint a fresh one
   (there is no renew by design).

## What this is NOT

- **Not a network error.** The daemon answered; it declined the token. A refused
  connection or timeout is a different problem (daemon not running, wrong port).
- **Not a missing grant.** A valid token without the grant a route needs gets
  `403` [MGS9015](MGS9015.md).

## See also

- [Tokens and grants](../../../concepts/tokens.md)
- `magus config mcp connector`: mint, list, and revoke connector tokens.
