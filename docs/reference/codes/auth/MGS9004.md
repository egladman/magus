...
- **Not a connector-token problem.** Connector tokens (for MCP clients) are a
  separate store; this is the built-in operator credential. See
  [MGS9001](MGS9001.md) for a rejected connector token.

## See also

- [auth diagnostics](README.md): the rest of the `MGS9xxx` range.
- [MCP](../../../guides/integrations/mcp.md): the bearer-token model the
  operator token authenticates against.
