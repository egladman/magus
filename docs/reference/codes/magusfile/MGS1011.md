...
```text
[MGS1011] producer: target "build": ctx.writesFiles declares an output
...
- The merge driver regenerates it during a conflicted merge, by looking up which
  project produces it.
- The dependency edge that makes `site` build _after_ its writer hangs off it.
...
cross-project _input_ side has always failed loudly on the same typo; outputs
