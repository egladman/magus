...
```text
hint: [MGS1028] 2 changed file(s) seed a project by directory containment while no
project declares them, so the targets they rerun were already correct: .golangci.yml,
mise.toml. Declare them in the owning project's sources, or leave them undeclared
deliberately (see ...)
```
...
- **Seeding** decides which projects `magus affected <target>` selects. A changed file
  seeds the project whose globs declare it; failing that, the project whose directory
  contains it.
...
**If the file is an input, declare it.** Declaring redirects the seed too: the file now
matches a project's globs, so it seeds by declaration rather than by containment, and this
diagnostic clears for it.
