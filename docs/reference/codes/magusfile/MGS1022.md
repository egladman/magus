...
`magus\ls`, `magus\targets`, `magus\affected`, `magus\graph`, `magus\where`, and
`magus\go_mod_replace_args` answer from the workspace already open on the
context, which is what makes them free of a subprocess. A script has no
workspace open.
...
- [Debugging](../../../guides/debugging.md#interactive-repl): `magus buzz
  --workspace`, the REPL with a magusfile actually loaded.
