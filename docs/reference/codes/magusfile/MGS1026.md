---
title: "MGS1026: a cacheable target reads a credential"
description: Fires when a target body calls magus.secret.read but the target can still replay from cache. A credential is not part of the cache key, so rotating it invalidates nothing and the target reports success without ever authenticating.
tags: [MGS1026, magusfile, secrets, cache, skip_cache, credentials, replay]
---

# MGS1026: a cacheable target reads a credential

A target reads a credential and is still cacheable:
