...
usually harmless, so magus only complains when a target's _entire_ output set
...
build order hangs off it. The owning project is scheduled to run _after_ this
