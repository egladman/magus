...
[MGS3012] aborting a stalled run: nothing has started, finished or printed a line for
  ...
  still admitted: docs:graph-generate, docs:index-generate
  stall window: 15m0s
...
The facts in the message are the whole investigation: the target that ran last, how long
ago that was, what else still held a seat at that moment, and the log holding whatever
the last target managed to print before it went quiet.

`still admitted` is the closest magus can get to "waiting on what?". A step blocked on a
dependency is blocked on something that is either still in flight, in which case it is
listed here, or already finished, in which case its absence says the wait itself is the
bug rather than the work. A list naming only the last step means everything else had
already handed its seat back.
...
The watchdog runs entirely inside the invocation. The server is an accelerant and never
