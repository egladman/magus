...
2. **If the descendant should inherit the parent's behavior** (the
...
- [sandbox diagnostics](README.md): the rest of the sandbox code range,
  including the `MGS2xxx` policy checks this audit sits alongside.
- [Sandbox model](../../../concepts/sandbox.md): the "descend only, never
  ascend" workspace-scope rule, and why this boundary is checked by audit
  rather than by the landlock policy.
