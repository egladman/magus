...
locale and terminal vars, plus the `MAGUS_*` server coordination vars.
...
succeeded; this message exists so you can spot when a tool's behavior
...
A trailing `*` makes a prefix pattern: `MISE_*` matches every name starting with
`MISE_`. The prefix must be at least three characters and end in `_`, since `GO*`
would also match `GOOGLE_APPLICATION_CREDENTIALS`; anything else is an error
([MGS2004](MGS2004.md)). It is intentionally not full glob syntax.
