...
tags: [MGS3010, ci, gate, redundancy, admission, broker, worktrees, exit 75, no-redundancy-check, gate_low_risk]
---
...
refusal unreachable in the case it was written for: the load reading came from the
server, ordinary commands ran without a persistent one, so an idle machine always took
...
- [The broker and the server](../../../guides/integrations/server.md): the process that owns the budget, and why losing it fails open.
