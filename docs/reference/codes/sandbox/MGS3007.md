...
mutating one project at once. Two unrelated runs never wait on each other: the
second exits 75, naming the holder. Two runs of one invocation tree, such as the
nested runs of parallel sibling steps, do wait: the holder finishes, the lock
frees, the waiter proceeds.
...
- `magus status`: names the lock's holder, including this case.
