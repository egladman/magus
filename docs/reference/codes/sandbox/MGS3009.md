...
description: Fires when a step is not started because the concurrency and declared memory_mb it needs do not fit in this machine's build budget alongside what other magus invocations hold. A budget they fill is refused at once, or after capacity_wait when one is set.
...
By default magus does not queue behind another magus invocation, so a budget they fill
is refused immediately, naming who holds it:
...
With `capacity_wait` set (`--capacity-wait 10m`), a step waits in the broker's line for
up to that long first, and the refusal says how long it waited:

```text
[MGS3009] not starting (root) ci after waiting 10m0s: this machine's build budget is
  full; ... held by pid 41221 (root) test (10.0 GiB), in /Users/x/repos/polish.
```

The same code, with the same exit 75, also refuses a step that would fit but would pass
an older waiter the line has already let others pass four times; the message names that
waiter. See [Waiting in line](../../../concepts/concurrency.md#waiting-in-line).
...
By default it refuses rather than queuing behind another invocation, so a step that
does not fit right now is told so immediately, naming who holds the budget, rather than
parked until room frees. Waiting on another invocation is something a run asks for,
with a bound (`capacity_wait`); the line it waits in cannot deadlock, because a waiting
step holds none of its claim until all of it is granted. What a step always waits for
is its own run: a step kept out by claims its own process or its own root invocation
holds waits for them, since refusing would fail a run over work it is already doing.
...
1. **Retry once a peer finishes, or wait for it.** The message names the pid, project,
   target and directory holding each claim, and `magus status` shows the whole budget
   across worktrees. Exit 75 is the signal a script or an agent should branch on to
   retry rather than investigate. `--capacity-wait DURATION` does the waiting in the
   broker's line instead, in arrival order.
