...
description: Fires when a step is not started because the concurrency and declared memory_mb it needs do not fit in this machine's build budget alongside what every other magus process holds. magus never waits on another magus process, so this is the only answer a full budget gives.
...
magus never queues behind another magus process, so a full budget is refused
...
It refuses rather than queuing. magus never waits on another magus process, so a step
that does not fit right now is told so immediately, naming who holds the budget, rather
than parked until room frees.
