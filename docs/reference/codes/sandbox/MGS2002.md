...
The sandbox denies writes outside the workspace, its private temp dir,
and the caches of the tools magus drives. This blocks the supply-chain
attack pattern of modifying shell startup files, sudoers, or cron tables
for persistence.

## Resolution

1. **If the write target is a legitimate cache** the default footprint
   does not know (a Bazel spell writing to `~/.cache/bazel`): add it to
   `magus.yaml`:
