...
description: A run whose standard input is written, through a chain of magus processes, by the run itself. Each stage would wait on the one before it, so the run is refused before it takes any lock.
...
When the chain of writers leads back to the run itself, every stage in the loop
