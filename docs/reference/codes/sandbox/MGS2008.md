---
title: "MGS2008: magus socket withheld"
description: Fires at debug level when magus withholds its socket vars from an op subprocess (independent of sandbox.enabled), or re-injects them into a recursive magus invocation.
tags: [MGS2008, sandbox, server, socket, env, recursive, security]
---

# MGS2008: magus socket withheld from subprocesses

The socket vars (`MAGUS_PROC_SOCKET`, `MAGUS_SERVER_ADDRESS`) are magus's own
pool pointers. magus withholds them from every op subprocess and re-hands them
only to a nested `magus`. This code is logged, at debug level only, on both
halves of that contract: when a var is withheld from a child, and when it is
re-injected for a legitimate recursive `magus` invocation.

```text
[MGS2008] withheld magus socket pointer(s) from op subprocess (done regardless of sandbox.enabled) (see https://github.com/egladman/magus/blob/main/docs/codes/sandbox/MGS2008.md)
...
The pool socket is unauthenticated: anything that can reach it can drive the
process behind it. If those vars were inherited by every child, a compromised
third-party spell could connect to the socket and issue commands. So magus
...
same trusted binary, and it genuinely needs the pool to coordinate. For that
case magus re-injects `MAGUS_PROC_SOCKET` and `MAGUS_SERVER_ADDRESS` as explicit
env overrides on the child, and records it with this code.
...
If a subprocess of yours genuinely needs to reach the server and cannot see the
...
`already running under a parent magus` error, which names the inherited
`MAGUS_PROC_SOCKET`, explains the opposite case where a program mistook the
