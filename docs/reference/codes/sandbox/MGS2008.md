...
description: Fires at debug level when magus withholds its socket vars from an op subprocess (independent of sandbox.mode), or re-injects them into a recursive magus invocation.
...
This withholding is **independent of `sandbox.mode`**. With the sandbox on,
