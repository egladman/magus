...
To ask whether a VCS exists at all without catching, read `vcs\name`. It is a
field, not a method, so it takes no call parentheses: it is `""` when nothing is
resolved and never raises - the detection half of the pair, the same split as
`os\env` and `os\lookupEnv`.
