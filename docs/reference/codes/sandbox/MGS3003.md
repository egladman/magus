...
description: proc.which could not resolve a command against PATH, so the tool a target depends on is not installed or not visible to this run.
tags:
...
`proc\which` could not resolve a command. The tool is not installed, or not
...
```text
[MGS3003] "docker" is not on PATH: exec: "docker": executable file not found in $PATH
```

## Why this raises instead of returning ""

`proc\which` used to answer `""` for a missing command so a magusfile could
branch on `proc\which(cmd) == ""`. That put the check on every call site and made
it optional - skip it and the empty string flows straight into an `proc\exec` or
...
```buzz
try {
    final p = proc\which("docker");
} catch (e) {
    if (e["code"] == "MGS3003") {
        magus\log.info("docker is not available; skipping the container checks");
    }
}
```
