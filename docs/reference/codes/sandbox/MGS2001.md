...
2. **If this is a legitimate spell** that needs to read an external path
   ...
   `mode` is `ro`, `rw`, `rx` or `rwx`. Use the narrowest mode that lets
   the workflow succeed.

3. **If you want to turn the sandbox off** for one invocation while you
   investigate:
   ...
   The flag, not `MAGUS_SANDBOX`: the variable is a floor that `magus.yaml` can
   raise and never lower.
