...
**MAGUS_SANDBOX_REQUIRED**
: When 1 or true, refuse to run (MGS2012) unless kernel landlock enforces the sandbox, instead of falling back to interpreter-level checks (MGS2005); needs sandbox.enabled. magus queue apply sets it for the regeneration it runs (default: false). Equivalent magus.yaml key: **sandbox.required**.
