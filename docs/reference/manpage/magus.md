...
**MAGUS_SHUTDOWN_GRACE**
: How long the broker (waiting for the runs holding it) and the server (waiting for the runs it cancelled to unwind) take to stop after a first SIGTERM; a second SIGTERM stops either at once, and 0 stops at once (default: 5m). Equivalent magus.yaml key: **shutdown_grace**.
