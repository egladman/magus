...
command, but applying the charm's [JSON Patch](../../charms.md#the-mechanism-a-json-patch-over-the-argv)
...
[charms.md](../../charms.md)). Magus validates a charm's _shape_ when the spell
...
- **An out-of-range index.** `{"op": "add", "path": "/7", ...}` against a
  ...
  updated (the reason the [`charm` constructors](../../charms.md#the-charm-constructor-reference)
...
- **Prefer a value anchor over a counted index.** Replace a literal `"/7"` with
  ...
  a stale count. See [the constructor reference](../../charms.md#the-charm-constructor-reference).
...
- [charms.md](../../charms.md): the anatomy of a charm and its JSON-Patch model.
- [charms.md § Previewing the rendered command](../../charms.md#previewing-the-rendered-command):
