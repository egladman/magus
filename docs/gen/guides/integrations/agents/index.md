...
behaviour: one source of truth to edit, and one to review.
...
It reads a command string and returns an opinion - enough to catch a habit,
useless against intent. `TestGuardKnownHoles` records what it cannot catch (a
command inside a script file, a program name from `$(...)` or a variable, an
alias, a recipe behind `make`) as executable fact.

The hook script and its response template are yours to edit, and editing them so
denials stop arriving is configuration, not a bypass - the same deal as disabling
every rule in `.eslintrc`. Enforcement lives below the guard:

| layer | role |
| --- | --- |
| guard hook | teaches: names the better command |
| [sandbox](../../concepts/sandbox.md) | enforces, where the OS allows (Landlock; `magus doctor` reports when it cannot) |
| `magus affected ci` | the real gate: drift, lint, test - committed, and reviewed |

A local template edit cannot touch the last row, which is why the first can
afford to be soft.

The question worth asking is not whether the agent can edit this config, but
where the config came from. One you wrote is yours; one that arrived in a cloned
repository is a stranger's code your host may run - the same standing risk as
that repo's `Makefile` or git hooks, and older than agents. Read it before you
run it.
