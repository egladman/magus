...
**Both are curated.** This is the part worth being precise about, because the
obvious guess is wrong: the simple form is not a summary, not a truncation, and
not generated by a model. There is exactly one hand-written body per skill, and
its author marks the spans that only the full form keeps. So the two can never
come to describe different behaviour - there is one source of truth to edit, and
one to review.
...
A skill is a bet about what the reader cannot infer, and that bet ages. Models
keep getting better at inferring the why, so the rationale that earns its context
today is the same text that is dead weight later - and a skill nobody revisits
quietly becomes a brick: bytes in every session that buy nothing.

The flag makes that reviewable instead of invisible. Re-asking "does this still
earn its context?" against the models you actually run is the audit; the two forms
are what let you answer it by installing one and comparing, rather than by
rewriting anything. If the simple form works for your host, the rationale in the
full one is documentation for humans rather than instruction for the model - which
is a useful thing to learn about your own skill set.

Start with the default. Reach for `--simple` when you have a specific reason: a
measured context budget, or a model you have watched succeed without the
rationale. It is a bet on the reader, so make it on evidence.
