...
- **Fail closed.** When the kernel layer is asked to enforce a policy and cannot, magus does not silently run unconfined; it either falls back to the in-process layer with a loud notice ([MGS2005](../reference/codes/sandbox/MGS2005.md)) or refuses the run ([MGS2010](../reference/codes/sandbox/MGS2010.md)).
...
A read outside every read rule raises [MGS2001](../reference/codes/sandbox/MGS2001.md); a write outside every write rule raises [MGS2002](../reference/codes/sandbox/MGS2002.md); an exec whose resolved binary path is outside every exec rule raises [MGS2007](../reference/codes/sandbox/MGS2007.md). The exec check runs against the path `exec.LookPath` returns, so an unqualified `curl` is checked at `/usr/bin/curl` (allowed), while a binary dropped in `~/.local/bin` is not.
...
On **macOS, Windows, or Linux older than 5.13** (or with the LSM disabled), `Apply` returns `ErrUnsupported`, and the run degrades to the **interpreter layer only**. This is a deliberate, non-fatal fallback: magus emits [MGS2005](../reference/codes/sandbox/MGS2005.md) once so the operator knows kernel enforcement is absent, then continues. `Supported()` reports `false` on non-Linux and checks `/sys/kernel/security/landlock` on Linux.
