...
|-------|----------|-----|---------|
| [`bash`](spells/bash.md) | Bash | 3 | Bash spell: shellcheck linting, shfmt formatting, and bats tests for shell scripts. |
| [`buzz`](spells/buzz.md) | Buzz | 3 | Buzz spell: check and test .buzz sources, plus run them through the magus interpreter. |
| [`golang`](spells/golang.md) | Go | 14 | Go toolchain spell: build, test, vet, fmt, gofumpt, mod-tidy, golangci-lint, and govulncheck as magus ops. |
| [`markdown`](spells/markdown.md) | Markdown | 3 | Markdown docs spell: markdownlint, prettier, and typos for linting, formatting, and spell-checking prose. |
| [`oci`](spells/oci.md) | - | 8 | OCI image spell: docker build, build-check, buildx, hadolint linting, and trivy/scout image scanning. |
| [`protobuf`](spells/protobuf.md) | Protobuf | 7 | Protobuf spell: buf build, lint, format, and code generation from .proto sources. |
| [`python`](spells/python.md) | Python | 11 | Python toolchain spell: pytest, ruff check/format, mypy, pyright, black, pip-audit, and uv build/sync as magus ops. |
| [`rust`](spells/rust.md) | Rust | 13 | Rust toolchain spell: cargo build, check, test, clippy, fmt, run, doc, bench, audit, and deny as magus ops. |
| [`sigstore`](spells/sigstore.md) | - | 6 | Sigstore spell: cosign sign, attest, verify, verify-attestation, and the blob pair for artifacts and images. |
| [`typescript`](spells/typescript.md) | TypeScript | 16 | TypeScript toolchain spell: tsc, eslint, prettier, vitest, jest, install, and audit run through the project package manager. |
<!-- END SPELL LIST -->
...
| **Vocabulary**          | the tool's own CLI command (`go-vet`, `cargo-clippy`, `tsc`, `eslint`, `ruff-check`) | magus's lifecycle (`build`, `test`, `lint`, `format`, `security`, `clean`, `generate`, `preflight`) |
...
magus\project.register(fun(p, cb) > bool { cb({ "spells": [golang] }); return true; });   // bind the spell (runs nothing)
...
export fun build(ctx: magus\Context, args: [str]) > void { golang["go-build"](ctx, { "cwd": "." }); }
export fun lint(ctx: magus\Context, args: [str])  > void { golang["golangci-lint-run"](ctx, { "cwd": "." }); }
export fun test(ctx: magus\Context, args: [str])  > void { golang["go-test"](ctx, { "cwd": "." }); }
```
...
- **Use the `::` escape hatch** (`magus run golang::go-vet api`) only for ad-hoc runs or introspection. The everyday surface is your composed targets.
...
Claims are also how spells settle **file ownership** between themselves. Two
spells may claim the same glob, and a broad claimant deliberately shuts a
narrow one out: the typescript spell claims `**/*.md`, `**/*.yaml`, and
`**/*.css` alongside its own extensions precisely so that, inside a TS
project, sibling single-file spells (markdown, a future json or css spell) do
not also claim those files - the web project is one unit, and its prose and
styles ride its cache key, not a second spell's. The corollary is a rule for
spell authors: **claimed implies required** - a file a spell claims must also
appear in its `needs`, or the claim shuts every other spell out while feeding
no cache key at all, and edits to that file invalidate nothing anywhere.
...
```buzz
import "magus/spell/golang";
magus\project.register(fun(p, cb) > bool { cb({ "spells": [golang] }); return true; });
```
...
magus\project.register(fun(p, cb) > bool { cb({ "spells": [golang, oci] }); return true; });   // co-bound

export fun build(ctx: magus\Context, args: [str]) > void {
    golang["go-build"](ctx, { "cwd": "." });
...
**Op keys are matched verbatim** (no kebab/case normalization, unlike target names), so a kebab key is reached by subscript in a magusfile: `golang["go-build"](ctx)`, not `go.build(ctx)`. An op whose key is a valid identifier (`pytest`, `eslint`) can use dot: `py.pytest(ctx)`. Either way the first argument is the target's context - the `ctx` the target function received - and any options follow it: `golang["go-build"](ctx, { "cwd": "." })`.
...
| Magusfile call   | `magus run go-build`                           | `golang["go-build"]()`            |
...
### Explicitness over magic (why `golang::gofmt` stutters)

The full-command convention is enforced even for streamlined toolchains like Go, where `go-build`/`go-test`/`go-fmt` all start with `go`. The result reads with a stutter on the CLI (`magus run golang::gofmt`), and that is **by design**:
...
- **The stutter shows up only in the escape hatch.** You type `golang::gofmt` for an ad-hoc op-direct run (see [targets.md](targets.md#cli-extension-spell-qualified-targets)). In normal use you compose `golang["gofmt"](ctx)` into a `format` target and run `magus run format`. magus favors explicitness over magic: the spell says exactly what it runs, and policy (which op is your "format") lives in your magusfile.
...
| **Op (operation)** | One tool-native action a spell exposes, named after its CLI command (`go-vet`, `golangci-lint`). Reached by subscript on the handle (`golang["go-vet"](ctx)`) or via `spell::op` on the CLI. See [Naming operations](#naming-operations).                         |
...
| [`github-actions`](../../spells/github/actions/spell.buzz) | cache backend + CI provider | A **remote cache backend** over the GitHub Actions Cache API in pure Buzz (bearer auth, byte-level chunked upload/streamed download, wired with `magus\cache.remote`) AND a **CI provider** emitting `::group::`/`::error::` workflow commands, wired with `magus\ci.provider`. |
| [`gitlab-ci`](../../spells/gitlab/ci/spell.buzz)  | CI provider     | A **CI provider** emitting GitLab's byte-exact `section_start`/`section_end` collapsible-log markers; the contract's worked example, and the one with in-file `test` blocks.                        |
...
| [`onepassword`](../../spells/onepassword/spell.buzz) | secret provider | A **secret provider**: one exported `resolve_secret` handler shelling out to the `op` CLI, wired with `magus\secret.provider`. See [secrets.md](secrets.md#spell-backed-providers) for setup and the sandbox passthrough its credentials need. |
