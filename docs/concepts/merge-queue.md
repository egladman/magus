...
- **Apply follows only the base's own validation.** A pull request's event runs the pull
  ...
  ([MGS3024](../reference/codes/sandbox/MGS3024.md)) any run but the `--workflow`
...
- **Apply pins what it can read itself.** The plan comes from validation's run, so apply
  ...
  ([MGS3025](../reference/codes/sandbox/MGS3025.md)) before merging anything that rests
...
refuses the run ([MGS3024](../reference/codes/sandbox/MGS3024.md)), downloading nothing,
