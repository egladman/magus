...
- **No credential reaches a hook's environment.** Hooks run with `MERGEQUEUE_TOKEN`,
  ...
  keeps the checkout's git config and the parent's environment out of its reach. Apply
  requires the kernel's enforcement of it: it sets `MAGUS_SANDBOX_ENABLED=1` and
  `MAGUS_SANDBOX_REQUIRED=1` for the regeneration, and on a host without landlock
  `apply --regenerate` refuses to start ([MGS2012](../reference/codes/sandbox/MGS2012.md)).
  The queue itself writes into a candidate only through its root, and never regenerates
  in a checkout where the change holds a symbolic link at or above a path it changes or
  regeneration writes. That is all it guarantees. The runner and the job's later steps still hold those tokens, and a
