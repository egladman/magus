...
- **No credential reaches a hook's environment.** Hooks run with `MERGEQUEUE_TOKEN`,
  `GITHUB_TOKEN`, `GH_TOKEN`, the GitHub Enterprise tokens and the Actions runtime tokens
  and URLs removed from their environment; main's regeneration in the apply job runs
  inside magus's sandbox, which keeps the checkout's git config and the parent's
  environment out of its reach. That is all it guarantees. The runner and the job's later steps still hold those tokens, and a
...
- **Nothing in a hook's environment steers a later step.** GitHub Actions' file commands
  ...
  later steps trust (`RUNNER_TEMP`, `RUNNER_TOOL_CACHE`, `GITHUB_ACTION_PATH`,
  `GITHUB_WORKSPACE`) are removed too; the rest of GitHub's variables are facts about
  the run and stay. Removing a path only unnames it: a hook runs as the runner's user,
  so the validation job is trusted for nothing after its first hook runs.
