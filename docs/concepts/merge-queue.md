...
- **No credential reaches a hook's environment.** Hooks run with `MERGEQUEUE_TOKEN`,
  ...
  environment. Main's regeneration in the apply job is not confined on the filesystem:
  it runs as the runner's user. The queue itself writes into a candidate only through its root, and never regenerates
...
the label prefix `"queue: "` and, with a setup, the committer
