...
files its merge needs regenerated by code the change touches, which its author
regenerates before queuing it again. A gate killed by a
...
A pull request that touches generated files merges like any other when merging it
leaves those files as the pull request has them: none conflicts, and main has not
changed any since the pull request forked from it. The queue regenerates nothing for it,
and the gate's drift check proves the files match what their generators make. It is
kicked back only when the merge needs them regenerated and the queue cannot do it
...
`merge-queue: needs regeneration`. Merge main in, regenerate, push, and queue it again:
the merge then leaves the files as the pull request has them.
...
- **Generated bytes come from main's regeneration, or from the author.** Where the
  ...
  (`KICK_REGENERATION`): its author merges main in, regenerates and pushes, review
  covers the result, and it is queued again. This is the only way generated bytes reach an
  author's branch. Generated files a change carries and its merge leaves alone are its
  author's bytes, merged as they are once the gate's drift check has held them to what
  their generators make.
...
| `KICK_REGENERATION`     | its merge needs regeneration its own code runs; its author does it   |
...
Planning kicks back, with `KICK_REGENERATION`, a change touching generated files that
its merge onto the base does not leave as the change has them (a conflict in one, or one
the base changed since the change forked) and whose regeneration the build tool cannot
prove runs none of the change's code (`--facts generation`, the proof apply needs
before it regenerates). Generated files the merge leaves alone need no regeneration and
keep the change queued. Apply does the same when it needs files regenerated that the
change does not touch and the proof fails.
