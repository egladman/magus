...
|-----|------|-------------|--------|
| `args` | `[str]` | Extra arguments appended to the resolved command. Omit it and a bare `docker["<op>"]()` forwards `magus run <target> -- <extra>` to the tool automatically; pass it to set the arguments explicitly, which replaces that passthrough. | [source](https://github.com/egladman/magus/blob/main/internal/interp/bindings/spell_object.go#L169) |
...
Lints the Dockerfile with -f gnu, so magus reads each finding's file, line and rule instead of prose. Declared here rather than on mgs_getTools' hadolint entry: only the invocation knows the format, since a wrapped tool can run other binaries under the same bin that report differently.
