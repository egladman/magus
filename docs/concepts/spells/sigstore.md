...
magus\project({ "spells": [sigstore] });

export fun attest(ctx: magus\Context, args: [str]) > void {
    sigstore["cosign-attest"](ctx, { "args": ["--predicate", "sbom.json", "--type", "cyclonedx", "app:latest"] });
}
```
...
export fun sign(ctx: magus\Context, args: [str]) > void {
    sigstore["cosign-sign"](ctx, { "args": ["app:latest"] });
}
```
...
export fun verify(ctx: magus\Context, args: [str]) > void {
    sigstore["cosign-verify"](ctx, { "args": ["app:latest"] });
}
```
