...
`magus agent install .agents/skills` followed by `magus agent install-agents-md`
equips Codex with Agent Skills and a managed always-on `AGENTS.md` section (two
subcommands, one per destination). Claude Code uses
