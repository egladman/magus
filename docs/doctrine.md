...
| denying a recursive grep of a bare identifier, to force `refs`         | measured and refused: 45 such greps in the whole tree, and `refs` answered roughly 60% of them; an advisory routed by the pattern's shape shipped instead, and it never blocks | `f963a9f1b`, measured with `9519797b3`                    |
