...
- A **service op** returns a `Service` — a long-running process. `command` (required) is the process, which `magus run <target>` forks in the **foreground and blocks** on (Ctrl-C signals it), rather than running to completion. `readiness` (a probe polled until it exits 0) and `stop` (a graceful-shutdown command) are **optional**; they are consumed once magus supervises processes in the background, so today only `command` runs. (`Service` is a distinct return type so the op's kind is inferred from what it returns.)
