...
> **Naming convention:** Teal/Lua binds each module per-import in `snake_case` (`local archive = require("magus.extra.archive")`, then `archive.some_method`). Buzz reaches them off the `import "magus/extra"` aggregate in `camelCase` (`extra.archive.someMethod`).
...
**Signature (Teal):** `archive.uncompress(src, dest, [opts]) → map[string]any`

**Signature (Buzz):** `extra.archive.uncompress(src, dest, [opts]) → map[string]any`
...
**Signature (Teal):** `archive.compress(src, dest, [opts]) → map[string]any`

**Signature (Buzz):** `extra.archive.compress(src, dest, [opts]) → map[string]any`
