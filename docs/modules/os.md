...
> **Naming convention:** Teal/Lua binds each module per-import in `snake_case` (`local os = require("magus.extra.os")`, then `os.some_method`). Buzz reaches them off the `import "magus/extra"` aggregate in `camelCase` (`extra.os.someMethod`).
...
**Signature (Teal):** `os.exec(cmd, [args], [dir], [opts]) → map[string]any`

**Signature (Buzz):** `extra.os.exec(cmd, [args], [dir], [opts]) → map[string]any`
...
**Signature (Teal):** `os.exec_sh(line, [dir], [opts]) → map[string]any`

**Signature (Buzz):** `extra.os.execSh(line, [dir], [opts]) → map[string]any`
...
**Signature (Teal):** `os.with_env(env, callback)`

**Signature (Buzz):** `extra.os.withEnv(env, callback)`
...
**Signature (Teal):** `os.with_slots(n, callback)`

**Signature (Buzz):** `extra.os.withSlots(n, callback)`
...
**Signature (Teal):** `os.platform() → string, string, string`

**Signature (Buzz):** `extra.os.platform() → string, string, string`
...
**Signature (Teal):** `os.exit(code)`

**Signature (Buzz):** `extra.os.exit(code)`
...
**Signature (Teal):** `os.sleep(ms)`

**Signature (Buzz):** `extra.os.sleep(ms)`
...
**Signature (Teal):** `os.which(cmd) → string`

**Signature (Buzz):** `extra.os.which(cmd) → string`
...
**Signature (Teal):** `os.stdin_is_terminal() → bool`

**Signature (Buzz):** `extra.os.stdinIsTerminal() → bool`
