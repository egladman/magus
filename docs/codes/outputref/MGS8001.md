...
description: Fires when magus query <ref> names a well-formed reference id but no captured output is stored for it, because it aged out of the cache or the ref is mistyped.
...
captured output, retrievable later with `magus query <ref>`. This code fires
