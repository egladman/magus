...
the warning fires. It is **observational** — magus does not block or reorder
execution.

The `--race` runtime detector (MGS4001) observes _actual_ file writes during
execution and can catch races that involve undeclared outputs. MGS4002 is the
cheaper, earlier companion: it catches _declared_ overlaps before any code
runs.
