...
[charms.md](../../charms.md)). Magus validates a charm's *shape* when the spell
...
The pointer is only resolved when the charm is *applied* to a concrete command.
...
- **Not a malformed patch.** A patch with a bad op name or a non-rooted path is
  ...
  *well-formed patch that does not apply* to this target.
