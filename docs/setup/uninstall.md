...
afterward sits where the [XDG Base Directory spec](https://specifications.freedesktop.org/basedir-spec/latest/) says to look. There
...
| Path                      | Default                 | Holds                                                                                                                                                                                                                                                                                                    |
| ------------------------- | ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `$XDG_STATE_HOME/magus/`  | `~/.local/state/magus/` | `v1.json` under `history/` (run history, read by volatility detection, the CI forecaster, and bisect), `pry_history` (REPL history), `x/` (the `magus x` picker, one file per project), `memory/` (per-repository agent memory), `mcp_token` and `connectors.d/` (secrets, one file per connector token) |
